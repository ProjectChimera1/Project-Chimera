...
using ProjectChimera.Multiplayer;          // MergedArrivalRing, TickCommandPacket, MergedTickPacket
...
namespace ProjectChimera.Sim.Tests.Server
{
    ...
    /// <see cref="MergedArrivalRing"/>, the extracted ring <c>LockstepManager.Flush</c> itself gates on (DW-390's
    /// extraction; DW-417's parallel MergedStreamGate was superseded by it at merge time) — through a real drop:
    ...
    ///   3. the client RESUMES stepping from arriving packets alone — <see cref="MergedArrivalRing.SeedEmpty"/> is
    ///      NEVER called after the match-start bootstrap (the asymmetry with a delay change, which DOES pre-seed,
    ///      is exactly the claim the CommitDelayChange doc makes and this test pins) — and every post-drop tick
    ...
    public class ClientDropFlushGateTests
    {
        ...
        /// <c>exec + Delay</c> once per exec tick, then advance only when the ring's stall/apply predicate
        /// (<see cref="MergedArrivalRing.IsReady"/> → consume, exactly Flush's shape) yields the merged packet for
        /// exec. The server is the real <see cref="MergedTickBuilder"/>; every built merged packet is delivered
        /// straight into the ring (<see cref="MergedArrivalRing.TryStore"/> — the HandleMergedTick path).
        /// </summary>
        private sealed class Harness
        {
            public readonly MergedTickBuilder Builder = new(2, SlotFaction);
            public readonly MergedArrivalRing Gate = new();
            ...
            public Harness()
            {
                // Match-start bootstrap (SeedBootstrap): ticks 0..Delay-1 are pre-seeded empty. This is the ONLY
                // seeding the whole test ever performs — the drop must unstall WITHOUT any further seed.
                Gate.SeedBootstrap(Delay);
            ...
            public void Submit(int slot, uint tick)
            {
                ...
                if (Builder.Submit(slot, _packBuf, len, out uint decoded))
                {
                    ...
                    if (Builder.TryBuild(decoded, out byte[] merged, out int mergedLen))
                        Gate.TryStore(merged, mergedLen); // server → client wire, straight into the arrival ring
                }
            }
            ...
            public bool Frame(bool remoteAlive)
            {
                ...
                if (!Gate.IsReady(ClientTick)) return false; // ── the Flush stall gate ──
                int len = Gate.LenFor(ClientTick);
                Gate.ConsumeArrival(ClientTick);             // ── Flush's post-apply consume ──
