...
namespace ProjectChimera.Sim.Tests.Dsl
{
    ...
    public class CustomEventIrTests
    {
        ...
        {
            var g = new TriggerGraph();
            (int root, DataWireType wire) = ExprParser.Parse("event.count * 10", g, NoVars, WaveParams);
            Assert.Equal(DataWireType.Int, wire);
            Assert.Contains(g.Nodes, n => n is ExprEventParamNode { Name: "count" });

            (_, DataWireType fixedWire) = ExprParser.Parse("event.rate", new TriggerGraph(), NoVars, WaveParams);
            Assert.Equal(DataWireType.Fixed, fixedWire);
            (_, DataWireType boolWire) = ExprParser.Parse("event.armed && true", new TriggerGraph(), NoVars, WaveParams);
        ...
        {
            var ex = Assert.Throws<JsonException>(() => ExprParser.Parse("event.ghost", new TriggerGraph(), NoVars, WaveParams));
        ...
        public void UnitDiesPayload_ReadsRefTypedParamsAsInt()
        {
            ...
            var g = new TriggerGraph();
            (_, DataWireType wire) = ExprParser.Parse("event.killer_faction == 1", g, NoVars, EventDispatchPlan.UnitDiesParams);
        ...
        {
            var g = new TriggerGraph();
            (int root, _) = ExprParser.Parse("event.count * 10", g, NoVars, WaveParams);
        ...
        {
            var g = new TriggerGraph();
            (int root, _) = ExprParser.Parse("event.count", g, NoVars, WaveParams);
