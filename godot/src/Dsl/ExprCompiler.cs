...
namespace ProjectChimera.Dsl
{
    ...
    {
        /// <summary>True when <paramref name="node"/> is one of the five 7.4 expression node kinds, the two
        /// 7.6 array expression kinds (expr_array_get / expr_array_len), or the 7.5
        ...
        public static bool TryCompile(
            ...
            => TryCompile(graph, rootId, declaredVars, inCondition, eventParams: null, out program, out error, arrayDecls);
        ...
        /// single-subscription rule). <paramref name="arrayDecls"/> keeps the 7.6 meaning documented on the
        /// overload above (the out-params disambiguate the two shapes positionally).
        /// </summary>
        public static bool TryCompile(
            TriggerGraph graph, int rootId,
            IReadOnlyDictionary<string, (DslValueType Type, VarScope Scope)> declaredVars,
