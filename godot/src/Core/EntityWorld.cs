...
namespace ProjectChimera.Core
{
    ...
    public enum UnitCommand : byte
    {
        ...
        UseItem    = 16, // Use the consumable in an inventory slot. WIRE: UnitId = hero entity, TargetX = inventory slot index (raw int). Handled by OrderApplier AFTER the IsAlive/FactionOf entity-ownership guard (UnitId names the hero ENTITY, so it is guarded like a normal unit command — the 3.15 anti-cheat that stops a player forcing an ENEMY hero to use items; NOT the Train/Revive building-command pre-guard pattern), delegating to ItemSystem.UseItemCommand. NEVER persists as a CommandState.
        DropItem   = 17, // Drop the item in an inventory slot back onto the ground. WIRE: UnitId = hero entity, TargetX = inventory slot index (raw int). Handled by OrderApplier AFTER the IsAlive/FactionOf entity-ownership guard (UnitId names the hero ENTITY, so it is guarded like a normal unit command — the 3.15 anti-cheat that stops a player forcing an ENEMY hero to drop items; NOT the Train/Revive building-command pre-guard pattern), delegating to ItemSystem.DropItemCommand. NEVER persists as a CommandState.
