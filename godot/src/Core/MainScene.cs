...
namespace ProjectChimera.Core
{
    ...
    public partial class MainScene : Node3D
    {
        ...
        internal void LoadSaveFromMenu(string slot)
        {
            ...
            // DW-467 (merge reconcile): a background save write may still be in flight (quit-to-menu right after a
            // save → Load Game of the same slot must see the NEW bytes). Same bounded wait as IssueLoad.
            _saveWriter?.WaitForIdle(10_000);
            byte[]? bytes = _ctx.SaveStore.Read(slot);
            if (bytes == null)
            {
