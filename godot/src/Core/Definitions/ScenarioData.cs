...
namespace ProjectChimera.Core.Definitions
{
    ...
    /// Story 7.9). FOLDED into <see cref="CanonicalModelHash"/> (v10, the 7.5 re-land merge) and therefore the
    /// <see cref="StartStateHash"/> seed — 7.5's original exclusion basis dissolved when 7.7's v8 folded the full
    /// trigger/DSL model, so divergent registries reject at the lobby handshake. The LIVE pending next-tick queue
    /// folds into per-tick <c>SimChecksum</c> (v18).
    /// </summary>
    ...
    public class ScenarioData
    {
        ...
        /// pre-7.5. The DECLARATIONS fold into <see cref="CanonicalModelHash"/> (v10, appended after the 7.8
        /// CustomUi fold) and therefore move the <see cref="StartStateHash"/> seed — divergent registries reject
        /// at the lobby handshake instead of desyncing in-sim (7.5's original exclusion basis dissolved when 7.7's
        /// v8 folded the full trigger/DSL model). The LIVE pending next-tick event queue folds into per-tick
        /// <c>SimChecksum</c> (v18).
        /// </summary>
