...
namespace ProjectChimera.CreationSuite
{
    ...
    public partial class BuildingCardPanel : Node
    {
        ...
        private void BuildUi()
        {
            ...
            // Right-docked, vertically centred, with a MARGIN gutter on the right. Explicit offsets (not Position)
            // pin all four edges so the box is exactly PANEL_W wide and ends MARGIN px from the screen edge. Crucially,
            // GrowHorizontal.Begin makes any content wider than PANEL_W grow LEFTWARD (into the screen) instead of
            // spilling off the right edge — the playtest cutoff. Height is pinned to PANEL_H; the inner ScrollContainer
            // handles the real body overflow.
            ...
            var closeBtn = ChimeraComponents.Button(EditorHotkeys.CloseLabel(EditorPanelId.BuildingCard), ChimeraComponents.ButtonVariant.Secondary, ChimeraComponents.ButtonSize.Sm);
