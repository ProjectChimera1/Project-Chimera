...
- **Editor delete→undo restore residue (Story 3.17).** `EntityWorld.SnapshotUnit`/`RestoreUnit` restore a deleted unit's def-derived fields automatically by re-routing its `SourceDefinition` through `ApplyUnitDefinition`. But **caller-owned** per-unit fields (set by a spawn path *after* the mapper — today `MeshType`/`GatherState`/`CarryCapacity`/`SupplyCost`, e.g. `DoSpawnWorker`'s overrides) are NOT def-derived, so a new one MUST be added to all three of `UnitSnapshot`, `SnapshotUnit`, and `RestoreUnit`'s residue replay, or it silently reverts to its `Create()` default on undo (the drop-debt class, relocated to the residue half). The round-trip guard in `ApplyUnitDefinitionGuardTest` is a hand-enumerated regression net, not an auto-coverage guard — this written rule is the coverage for a *newly added* residue field.
