...
- **Save/Load Interface (FR-7d/e)** — a richer-than-WC3 product surface: a visual **hero-picker menu** (icon + basic info per save, save/load/overwrite, multiple profiles per player), provided as a **reusable platform component** the creator enables per scenario. This makes the profile object richer than a raw blob (needs display metadata: icon ref, name, level, summary stats). UI work lands with M2 authoring; server-backed profile listing with M5. Belongs in the `gds-ux` pass.
