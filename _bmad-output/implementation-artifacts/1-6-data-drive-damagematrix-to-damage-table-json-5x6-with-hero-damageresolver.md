...
- [x] [Review][Defer] Loader does not fully fail-closed on duplicate JSON keys [DamageTable.cs:115-142] — a duplicated row/cell key in `damage_table.json` silently resolves **last-wins** (`System.Text.Json` de-duplicates; `row.Count`/`Multipliers.Count` still equal `COUNT`, so all validation passes and the earlier value is silently dropped), contrary to AC4's "never silent." Verified with a standalone .NET 8 repro (Blind + Edge). Not triggered by the shipped table; a proper fix needs `JsonDocument`/`Utf8JsonReader` duplicate-key detection (STJ does not reject duplicate dictionary keys natively). **Deferred (review decision 2026-06-23):** future-content-only edge case; shipped table has no duplicate keys; the data-authoring UI is the natural validation point — out of scope for 1.0's determinism floor. See `deferred-work.md`.
- [x] [Review][Patch] `DamageResolver.Apply` has no aliveness guard — latent phantom-kill (duplicate `UnitKilled` event + inflated `RecordKill`) if a future caller invokes it on an already-dead / ≤0-HP target; not triggerable today (all 3 call sites guard upstream — verified), but the resolver is the documented single reusable damage path, and later epics (abilities, DoT via the Effect-Graph) will add callers. Safe one-line guard `if (!world.IsAlive(t)) return false;` at the top of `Apply` — no-op for current goldens, so AC2-preserving. [DamageResolver.cs:51-66] **✅ Applied 2026-06-23** — guard + explanatory comment added at top of `Apply`; game build 0 errors, Tier-1 suite 97/0, goldens byte-identical (`git status` clean on `*golden*.txt`).
