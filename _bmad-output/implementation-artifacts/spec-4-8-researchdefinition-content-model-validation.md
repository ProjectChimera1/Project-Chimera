...
**Problem:** Research has no data-driven authoring surface yet — a creator cannot declare a faction-wide, repeatable, timed upgrade with per-level cost/time/modifier deltas, and buildings cannot declare which research they make available. No runtime order path exists yet (4.9); this story is the content model + import-time validation only.
...
- Numeric content-model fields stay `float`/`int` (authoring values) — no `Fixed` conversion in this story; that's the single load-boundary 4.9 owns.
...
**Never:**
- No `ModifierStore`/`ModifierSystem`/`BuildingSystem` runtime wiring, no order path (4.9).
- No `SimChecksum` fold, no golden re-baseline (4.10 — this story adds no mid-match-mutable state).
- No command-card UI, no research authoring editor panel (4.11).
...
- addressed_findings:
  ...
  - `[low]` `[patch]` No character-set sanitization existed for research ids, unlike every other id-bearing content type (`UnitDefinitionValidator.SanitizeId`), inviting friction for 4.11's future editor (widget keys, JSON-key safety). Added the same `[a-z0-9_]` gate. Flagged by the Blind Hunter review layer.
  ...
  - Rejected (8, all low): the epic context's general `Validated<T>` framing vs. this story's throw-based gate (Intent Alignment — already resolved with evidence in this spec's Design Notes, favoring the AC's specific wording over the epic's looser paraphrase); zero research entries exist in shipped faction content yet (Intent Alignment — correctly scoped to Story 4.11); research cost validation is pinned to `{"ore","crystal"}` rather than the scenario's N-resource registry (Intent Alignment — deliberately chosen, see Boundaries); `DetectCycle` rebuilding its id-set independently of `Validate` (Blind Hunter — mirrors `TechTreeValidator`'s existing, accepted pattern, not a new defect); no `FactionWriter.SyncFactionResearch` write-back path exists yet (Blind Hunter — explicitly out of scope per this spec's Never section, Story 4.11's job); `MaxResearchPerFaction = 64`'s justification (Blind Hunter — already acknowledged as a judgment call in this spec's Design Notes); dense cross-referencing XML doc comments risking future rot (Blind Hunter — stylistic, no concrete failure scenario); `ResearchDefinition.DisplayName` left unvalidated (Blind Hunter — cosmetic display text, no functional consumer yet).
...
**Why cost/time live per-level, not on `ResearchDefinition` itself:** 4.9's AC says "the next level's cost/time apply" after each completion, so each `ResearchLevel` carries its own `Cost`/`TimeTicks`; `CancelRefundFraction` stays definition-level since 4.9 refunds `CancelRefundFraction × currentLevelCost` — one fraction, applied to whichever level was in progress.
...
- No runtime consumer exists yet for `Research`/`AvailableResearch`/`ResearchValidator` (by design — 4.9/4.11 own that), so this content model is currently inert except at import-time validation.
...
- **[low]** Added a symmetric ±`Range` (16.16 Fixed ceiling) check to the four `ResearchModifierDelta` floats — they quantize into the same Fixed fields as level costs (already range-checked), so a finite-but-out-of-range value (`100000`) would have overflowed at 4.9 quantization. Plus over-range and in-range-negative tests.
