...
- source_spec: `_bmad-output/implementation-artifacts/spec-11-4-under-attack-alerts-minimap-pings-event-cues-denial-acknowledgment-feedback.md`
  ...
  evidence: Found by the post-merge ultra-review (2026-07-29), `MinimapBridge.cs:248`, then **CONFIRMED IN-ENGINE** the same day during the minimap-layout gate: an Alt+LMB at minimap px (1750, 1030) both dropped the ping AND moved the camera pivot to `(-79.36, 0, 74.24)` — the exact world point clicked — so the camera-view box jumped to sit under the new ping. Now directly observable, since the minimap layout fix put the panel on screen. Fix shape: track that the press was consumed as a ping and skip the pan/drag branch until the button releases.
