...
<!-- 1.12 is the FIRST post-M1 DG (design-gap) story. M1 [1.1–1.10c] is GREEN and 1.11 verified the four
     ...
     The work is BROWNFIELD: ~4 small additive sim slices (enum + SoA arrays, CombatSystem branch split,
     MovementSystem Hold anchor, both command-apply switches) + 1 presentation slice (SelectionSystem) + the
     SimChecksum re-baseline that DG stories 1.12/1.13 were always scheduled to do. The single biggest
     hazard is the SimChecksum fold: it is a GLOBAL change that re-baselines EVERY golden and trips three
     version-pin guard tests on purpose — do it ONCE, last, exactly as documented in Task 6. -->
...
- **AC1a:** `UnitCommand` gains `AttackTarget`, `Patrol`, `Follow` appended AFTER `Build=5` (so `AttackTarget=6, Patrol=7, Follow=8`). Values `0–5` are UNCHANGED (a pre-1.12 `.chmr` replay byte still means exactly what it meant). Update the enum doc comment, and fix the now-false `HoldPosition = 4, // Same as Stop (Phase 1)` comment (`EntityWorld.cs:16`).
- **AC1b:** Each genuinely-new per-entity field is a NEW parallel SoA array on `EntityWorld`, sized `MAX_ENTITIES`, with a sentinel set in BOTH the ctor (`Array.Fill`) AND `Create()` (per-slot reset — a recycled slot must never carry stale data). Minimal set (see [SoA design](#soa-design-the-new-arrays-what-to-reuse)): `CommandTarget[]` (`int`, sentinel `-1` — the entity the command references: enemy id for AttackTarget, friendly id for Follow) and `PatrolAnchor[]` (`FixedVec3` — the "other" patrol waypoint). The follow leash is a `Fixed` CONSTANT in `CombatSystem`, not a per-entity array.
...
- **AC4a (Patrol):** A `Patrol` unit moves between two waypoints: `CommandGoal` (current-leg target) and `PatrolAnchor` (the other end). It engages enemies en route exactly like AttackMove (reuse that combat logic), then resumes toward `CommandGoal`; on arrival it SWAPS `CommandGoal ↔ PatrolAnchor` and heads back (the "reverse at the final leg"). The 2-waypoint ping-pong is the in-scope model (see [Patrol model decision](#decisions-baked-in-override-before-dev-story-if-you-disagree)). Deterministic: arrival uses a `Fixed` squared-distance threshold; the swap is pure state mutation.
...
- **AC6a (both switches):** The command-apply switches in BOTH `LockstepManager.ApplyOrders` (`:650–700`) AND `ReplayPlayer.ApplyOrders` (`:168–218`) gain cases for `AttackTarget`, `Patrol`, `Follow` (HoldPosition is already handled by the Stop/Hold apply case — leave that). The two switches MUST stay in lock-step: an order handled in one but not the other desyncs replay-vs-live. A test asserts both produce identical post-apply world state for each new order.
- **AC6b (serialization round-trip):** The new orders serialize/deserialize identically through both wire paths with NO format change — `AttackTarget`/`Follow` pack the target ENTITY id into the existing `UnitOrder.TargetX` int via `Fixed.FromRaw(id)` (read back as `o.TargetX` directly, never `.ToFloat()`); `Patrol` uses `TargetX/TargetZ` as a ground point like Move. A round-trip test through `TickCommandPacket.Write`→`TryRead` (live) and `ReplayRecorder`→`ReplayPlayer` (file) reproduces the order. `ReplayRecorder.VERSION` is NOT bumped (the 11-byte format is unchanged).
- **AC6c (SimChecksum fold + re-baseline):** The new SoA arrays (`CommandTarget`, `PatrolAnchor`) fold into `SimChecksum.Compute` (entity loop), `SimChecksum.AlgoVersion` bumps `3 → 4`, the three version-pin guards are updated (see [the re-baseline surface](#the-simchecksum-re-baseline-surface-task-6--do-this-once-last)), and ALL existing goldens + the NEW 1.12 golden are re-recorded via the `CHIMERA_GOLDEN_RECORD` flow. A differential test proves the new fields actually move the hash (mutate one → checksum changes).
...
### ✅ IN scope (this story)
1. **Enum + SoA storage** — append `AttackTarget`/`Patrol`/`Follow` after `Build`; add `CommandTarget[]` + `PatrolAnchor[]` SoA arrays with sentinels in ctor + `Create()`.
...
- **NO rally points / queued (shift-click) command chaining / multi-waypoint (>2) patrol routes.** Patrol is a 2-waypoint ping-pong. Multi-waypoint needs per-unit dynamic lists (breaks SoA) — explicitly deferred.
...
- **Do NOT leave float in sim.** All distance/leash/range/swap math uses `Fixed` (16.16). The packed target id rides in `TargetX` as a raw int via `Fixed.FromRaw(id)` / read as `o.TargetX` — NEVER `Fixed.FromFloat`/`.ToFloat()` for an entity id (that path is float and would corrupt the id and break determinism).
...
|---|---|---|
| `UnitCommand` enum | Idle/Move/AttackMove/Stop/HoldPosition/Build (`EntityWorld.cs:10–18`) | Append AttackTarget/Patrol/Follow (6/7/8) |
| EntityWorld SoA | `AttackTarget[]` (transient live target, `-1`), `CommandGoal[]`, `MoveTarget[]`, `Flags` (`:81–145`) | NEW `CommandTarget[]` + `PatrolAnchor[]` |
...
| SimChecksum | v3: Position+Health+buildings+resources+RNG (`SimChecksum.cs`); **does NOT hash any command field** | Fold `CommandTarget`+`PatrolAnchor`; AlgoVersion 3→4 |
...
- [ ] **Task 1 — Enum extension + new SoA arrays (AC: 1a, 1b).**
  - [ ] In `godot/src/Core/EntityWorld.cs`, append to `UnitCommand` (after `Build = 5`): `AttackTarget = 6`, `Patrol = 7`, `Follow = 8` — each with a one-line doc comment. Do NOT renumber 0–5. Fix the stale `HoldPosition = 4, // Same as Stop (Phase 1)` comment to describe the new true-hold semantics.
  - [ ] Add two SoA arrays: `public readonly int[] CommandTarget;` (entity id this command references — enemy for AttackTarget, friendly for Follow; `-1` = none) and `public readonly FixedVec3[] PatrolAnchor;` (the "other" patrol waypoint). Allocate both in the ctor (`new int[MAX_ENTITIES]` / `new FixedVec3[MAX_ENTITIES]`), add `CommandTarget` to the `Array.Fill(..., -1)` sentinel block (`:197–199`), and reset BOTH in `Create()` (`CommandTarget[id] = -1; PatrolAnchor[id] = position;`). **Skipping the `Create()` reset is the classic SoA bug — a recycled slot carries the previous unit's target → nondeterministic ghost behavior.**
  - [ ] Document the reuse vs new decision in code comments: `AttackTarget[]` stays the TRANSIENT live target (set each tick by the spatial-hash query); `CommandTarget[]` is the PERSISTENT player-issued forced/follow target. `CommandGoal[]` holds the current move/patrol-leg destination (reused).
...
- [ ] **Task 2 — CombatSystem: split Hold from Stop + three new branches (AC: 1c, 3, 4a, 4b, 5a).**
  ...
  - [ ] `TickPatrolCombat` (AC4a): same combat body as `TickAttackMoveCombat` (engage in-range enemies, resume toward goal after) BUT on goal-arrival, instead of `→ Idle`, SWAP `CommandGoal[i] ↔ PatrolAnchor[i]` and keep moving (reverse). Factor the arrival test (`AMOVE_ARRIVE_SQR`) so both AttackMove and Patrol share it. Determinism: swap is pure; threshold is `Fixed`.
  ...
  - [ ] Extend the gatherer-normalization guard (`:68–75`): a gatherer issued `AttackTarget`/`Patrol`/`Follow` must also normalize to `Idle` (same invariant as the existing AttackMove/Stop/Hold normalization — a gatherer must never sit in a combat command no system completes).
...
- [ ] **Task 4 — Both command-apply switches + serialization (AC: 6a, 6b).**
  - [ ] In `godot/src/Multiplayer/LockstepManager.cs` `ApplyOrders` (`:650–700`), add cases:
    ...
    - `case UnitCommand.Patrol:` → set `CommandGoal[id]` + `MoveTarget[id]` to `(TargetX,TargetZ)` (a ground point, like Move), set `PatrolAnchor[id] = world.Position[id]` (the issue-point becomes the return anchor), set `Moving`, clear `Attacking`. Optionally call `OnRequestAttackMove` (patrol engages like attack-move; the fixed waypoint can take a path).
    ...
    - Note: `_world.CommandState[id] = o.Command;` is already set BEFORE the switch (`:659`), so each new case only sets the EXTRA payload.
...
- [ ] **Task 5 — SelectionSystem presentation: right-click enemy / P / F (AC: 2, 4 issue-half).**
  ...
  - [ ] Add `IssuePatrolCommand(Vector2 screenPos)` (mirror `IssueAttackMoveCommand` but command `Patrol`; single destination — NO formation grid here, that's 1.13) and a `P`-key handler in the keyboard block (`:239–257`).
...
- [ ] **Task 6 — SimChecksum fold + AlgoVersion bump + re-baseline ALL goldens + new 1.12 golden (AC: 6c, 6d). ⚠ GLOBAL change — do this ONCE, LAST, after Tasks 1–5 are behavior-complete.**
  - [ ] In `godot/src/Core/SimChecksum.cs`: in the entity loop (`:53–62`, after `Health`), fold the two new arrays — `hash = Mix(hash, world.CommandTarget[i]);` and the three `PatrolAnchor[i].{X,Y,Z}.Raw` mixes. Bump `AlgoVersion` `3 → 4` (`:37`) and add a `v4 — Story 1.12: fold CommandTarget + PatrolAnchor (full command vocabulary)` line to the version doc.
  - [ ] Update the three version-pin guards (they break ON PURPOSE — that is their job):
    ...
    - Add a differential assertion (extend this guard or a new test): mutating `CommandTarget`/`PatrolAnchor` on a live entity MUST change `SimChecksum.Compute` — proving the new fields are actually folded (the EntityWorld analogue of the existing ResourceStore coverage guard, which only reflects `ResourceStore`).
  ...
  - [ ] Add `godot/ProjectChimera.Sim.Tests/Golden/CommandVocabularyScenario.cs` (mirror `GoldenScenario.cs`/`AiActiveScenario.cs`): an in-code, all-`Fixed` scenario that issues and exercises EVERY order — at least one unit each on Move, AttackMove, Stop, HoldPosition, AttackTarget (forced onto a specific enemy with a nearer decoy present), Patrol (two waypoints + an enemy on the lane), Follow (a friendly that moves). Drive via `SimulationHost.Create(NullLogSink.Instance, new FactionRegistry(2), …)`, `ChecksumInterval=1`, `DefaultTicks=300`. Author all command state directly (call the same `EntityWorld`/apply logic the switches use, or set SoA fields directly) — no Godot, no LLM.
...
- [ ] **Task 7 — Behavior unit tests (AC: 1c, 2, 3, 4, 5).**
  - [ ] Add `godot/ProjectChimera.Sim.Tests/Combat/CommandVocabularyTests.cs` (new `Combat/` folder, mirroring the `Multiplayer/`/`Validation/` convention). Build a small `EntityWorld` + a `CombatSystem` (and a `MovementSystem` where needed) directly — no Godot:
    ...
    - **AC4a patrol:** two pinned waypoints, no enemies, step N ticks → the unit ping-pongs and reverses (assert it returns toward the anchor after reaching the goal); with an enemy on the lane → it deals damage then resumes.
...
| `PatrolAnchor[]` (NEW) | `FixedVec3` | The "other" patrol waypoint; `CommandGoal` holds the current leg; swap on arrival. | YES (3 mixes) |
...
| `CommandGoal[]` (reuse) | `FixedVec3` | Current Move/AttackMove/Patrol-leg destination. | No (as today) |
| `FOLLOW_LEASH` (const) | `Fixed` in CombatSystem | Re-path threshold for Follow. Not per-entity → a named constant, not an array. | No |

Why one `CommandTarget` for both AttackTarget and Follow: a unit is in exactly one `CommandState` at a time, so the enemy-id and friendly-id uses never overlap. This keeps the SoA addition minimal (1 int + 1 vec) and the checksum fold small. (If you prefer two explicitly-named arrays, that's fine too — but then hash BOTH and update the golden/known-state accordingly.)
...
**Edit (sim):**
- `godot/src/Core/EntityWorld.cs` — enum append + 2 SoA arrays + sentinels (Task 1).
...
1. **Patrol = 2-waypoint ping-pong** (`CommandGoal` ↔ `PatrolAnchor`), not an N-waypoint route. Rationale: multi-waypoint needs per-unit dynamic lists, which break the SoA model and pull in queued-command machinery that's explicitly out of scope. The AC's "two or more waypoints … reverses at the final leg" is fully satisfied by a 2-point ping-pong (every arrival reverses).
