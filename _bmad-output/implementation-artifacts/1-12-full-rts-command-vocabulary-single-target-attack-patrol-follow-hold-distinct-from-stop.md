...
- [x] **Task 4 — Both command-apply switches + serialization (AC: 6a, 6b).**
  ...
  - [x] Mirror the SAME three cases EXACTLY in `godot/src/Multiplayer/ReplayPlayer.cs` `ApplyOrders` (`:168–218`). **A case added to one switch but not the other is a guaranteed replay-vs-live desync — this is the #1 trap of this story (the epic note calls it out).**
...
- [x] **Task 7 — Behavior unit tests (AC: 1c, 2, 3, 4, 5).**
  ...
  - [x] All test scenarios authored in `Fixed` (no `Fixed.FromFloat`), ascending-id, no wall-clock — they run on every OS including the WSL leg.

- [x] **Task 8 — Regression, code review, sprint status.**
  - [x] Run the full Windows CI command `dotnet test godot/ProjectChimera.Sim.Tests/ProjectChimera.Sim.Tests.csproj -c Release` → green (was **239** at 1.11 close; **do not hardcode** — rely on exit code). Confirm: the 5 existing goldens re-recorded to `checksum_algo_version: 4`, the new 1.12 golden added, all three version-pin guards green at v4, `SystemOrderTest` untouched/green, the analyzer gate green (advisory on master).
  ...
  - [x] Run `gds-code-review` (3-layer adversarial, fresh-context / different LLM). On PASS, set this story `done` in `sprint-status.yaml` and update `last_updated`. (This workflow only sets `review`; code-review flips `done`.)
...
- Tier-1 behavior-neutrality checkpoint (after Tasks 1–5, BEFORE the Task 6 checksum bump): full Release suite **239 pass / 1 skip / 0 fail** — the v3 goldens stayed byte-identical, proving the `OrderApplier` extraction + the Hold/Stop split + the MovementSystem Hold-anchor are behavior-neutral for every existing scenario (no existing golden/test uses Hold/Patrol/Follow/AttackTarget — verified by grep).
- One test-setup bug found & fixed during Task 7: `HoldAndStop_BothAttackInRangeEnemy` used `range: 3` (> MELEE_THRESHOLD 2.5 → projectile, not instant melee), so a combat-only test (no ProjectileSystem tick) saw no damage. Fixed to melee `range: 2`. Not a sim-logic defect.
- Task 6 re-baseline: recorded all goldens with `CHIMERA_GOLDEN_RECORD=1` (filter `~Golden`, **56 pass**), rebuilt, re-verified normally. Known-state v4 hash captured from the intentional placeholder-fail run: `0x964FF9F8` (re-pinned).
- Final full Tier-1 Release: **263 pass / 1 skip / 0 fail** (was 239 → +24 new tests). Full Godot game build (`godot.csproj`, Debug): **0 errors**.

### Completion Notes List

**All 6 ACs met; all 8 tasks complete.**

- **AC1 (enum + SoA + branch-routing):** `UnitCommand` gained `AttackTarget=6, Patrol=7, Follow=8, PatrolAppend=9` after `Build=5` (0–5 frozen); the stale `HoldPosition // Same as Stop` comment is corrected. New SoA on `EntityWorld`: `CommandTarget[]` (one array serves AttackTarget enemy-id + Follow friendly-id, mutually exclusive) + the patrol-route ring `PatrolWaypoints`/`PatrolCount`/`PatrolIndex`/`PatrolDir` + `MAX_PATROL_WAYPOINTS=8`; sentinels in ctor (`Array.Fill(CommandTarget,-1)`) AND `Create()` (the recycled-slot reset). Branch-routing proven behaviorally (`NewCommands_RouteToDistinctBranches_NotIdleFallThrough`).
- **AC2/AC3 (AttackTarget):** `TickAttackTargetCombat` force-fires only `CommandTarget`, ignoring nearer enemies; on dead/invalid target it clears the slot, drops to `Idle`, and re-acquires the same tick (delegates to `TickIdleCombat` — and deliberately does NOT double-tick the cooldown). `SelectionSystem` right-click hit-tests an enemy (`FindNearestEnemyUnit`) → `AttackTarget`; ground/friendly → existing `Move`.
- **AC4 (Patrol/Follow):** `TickPatrolCombat` mirrors AttackMove's engage-then-resume, with `ResumePatrol` advancing the index and **reversing at both ends** (pure-integer arithmetic, `Fixed` arrival threshold); `P` arms patrol placement, Shift-click appends via `PatrolAppend`. `TickFollowCombat` re-paths beyond `FOLLOW_LEASH` (3u, named const), idles within, drops to `Idle` on the leader's death; `F` arms follow. Follow is tracking-only in 1.12 (Decision #3).
- **AC5 (Hold ≠ Stop):** combat split into `TickStopCombat`/`TickHoldCombat` (both delegate to a shared `TickStationaryCombat` body — DRY + behavior-neutral); the REAL teeth is the `MovementSystem` Hold-anchor — a `HoldPosition` unit zeroes velocity and skips seek+separation, so it is never displaced, while Stop still gets pushed (`HoldPosition_NeverDisplacedBySeparation_WhereasStopIs`).
- **AC6 (both paths + golden + checksum):** the formerly-duplicated apply switch was extracted to a single Godot-free `OrderApplier.Apply` (in `NetworkCommand.cs`) that BOTH `LockstepManager.ApplyOrders` and `ReplayPlayer.ApplyOrders` now call — so replay-vs-live parity is **structural** (one switch; the epic's #1 desync trap is now impossible) and Tier-1-testable. Wire format unchanged (target id packs into `TargetX` via `Fixed.FromRaw`, read as `o.TargetX`; `ReplayRecorder.VERSION` stays 2). `SimChecksum` folds the new fields (count-driven for waypoints), `AlgoVersion 3→4`; all 3 version-pin guards updated (`VersionStampConsistencyTests`, the coverage-guard known-state hash re-pinned `0x8C96EC08→0x964FF9F8` + rename V3→V4, + a new EntityWorld differential-fold check); all 5 existing goldens re-recorded to v4 + the new `command-vocabulary-scenario.golden.txt` (300 evolving samples) added & embedded.

**Cross-platform (AC6d / Task 8):** the new golden is **NOT Windows-gated** — every hashed field is `int`/`Fixed.Raw` and Player2 is left empty so the float-scoring AI no-ops (integer-gated, identical to `GoldenScenario`'s proven cross-platform-safe pattern). So the normal Tier-1 run compares it on BOTH CI legs; the always-on `ubuntu-latest` leg verifies it Win↔Linux on push. A manual WSL run was NOT done here because `cross-platform-determinism-check.ps1` clones committed HEAD (this work is uncommitted) — it would test stale code; CI covers it post-commit.

**Out-of-scope fences honored:** no formation/group-move changes (1.13); no general shift-queue (patrol routes only, fixed-cap ring); AttackMove not rewritten; `UnitCommand` 0–5 / the 11-byte wire / `ReplayRecorder.VERSION` / `CanonicalModelHash` (=2) unchanged; no new system (`SystemOrderTest` untouched & green); no NuGet/CI-job edits; no float in sim. The `AttackDamage==0` early-`continue` edge (a zero-damage unit can't Patrol/Follow) is flagged-not-built per the story (Epic-2 concern).

**Decision deferred to code review:** `gds-code-review` has NOT been run — this workflow's terminal state is `review`. A fresh-context, different-LLM `gds-code-review` is the next step and is what flips the story to `done` (Task 8, subtask 3).

### File List

**Edit (sim):**
- `godot/src/Core/EntityWorld.cs` — enum +4 values, `MAX_PATROL_WAYPOINTS`, `CommandTarget` + patrol-route ring SoA, sentinels (ctor + `Create`).
- `godot/src/Combat/CombatSystem.cs` — Hold/Stop split (`TickStationaryCombat`), new `TickHoldCombat`/`TickAttackTargetCombat`/`TickPatrolCombat`/`TickFollowCombat`/`ResumePatrol` + `FOLLOW_LEASH`, extended gatherer normalization, class summary.
- `godot/src/Navigation/MovementSystem.cs` — Hold-position separation anchor.
- `godot/src/Multiplayer/NetworkCommand.cs` — new shared `OrderApplier.Apply` (the single command→world switch) + `#nullable enable`.
- `godot/src/Multiplayer/LockstepManager.cs` — `ApplyOrders` delegates to `OrderApplier.Apply`.
- `godot/src/Multiplayer/ReplayPlayer.cs` — `ApplyOrders` delegates to `OrderApplier.Apply`.
- `godot/src/Core/SimChecksum.cs` — fold `CommandTarget` + patrol-route ring, `AlgoVersion 3→4` + docs.

**Edit (presentation):**
- `godot/src/UI/SelectionSystem.cs` — right-click enemy hit-test, `P`/`F` arming + click consumption, `FindNearestEnemyUnit`, `IssueAttackTargetCommand`/`IssuePatrolCommand`/`IssueFollowCommand`, `EnqueueTargetedCommand`, input legend.

**Edit (tests / guards / project):**
- `godot/ProjectChimera.Sim.Tests/Meta/VersionStampConsistencyTests.cs` — `ExpectedSimChecksumAlgoVersion 3→4`.
- `godot/ProjectChimera.Sim.Tests/Golden/SimChecksumCoverageGuardTest.cs` — rename V3→V4, re-pin `0x964FF9F8`, new `EntityCommandFields_AreFoldedIntoTheChecksum`.
- `godot/ProjectChimera.Sim.Tests/ProjectChimera.Sim.Tests.csproj` — embed `command-vocabulary-scenario.golden.txt`.
- `godot/ProjectChimera.Sim.Tests/Golden/{golden-scenario,golden-multifaction,golden-applier-scenario,same-tick-tie-break,ai-active-scenario}.golden.txt` — re-recorded to `checksum_algo_version: 4`.

**Create (tests):**
- `godot/ProjectChimera.Sim.Tests/Combat/CommandVocabularyTests.cs` — AC1c/2/3/4/5 behavior.
- `godot/ProjectChimera.Sim.Tests/Multiplayer/CommandApplyParityTests.cs` — AC6a parity + AC6b serialization round-trips.
- `godot/ProjectChimera.Sim.Tests/Golden/CommandVocabularyScenario.cs` — all-orders in-code scenario.
- `godot/ProjectChimera.Sim.Tests/Golden/CommandVocabularyGoldenTests.cs` — AC6d two-run + golden + record hook (NOT Windows-gated).
- `godot/ProjectChimera.Sim.Tests/Golden/command-vocabulary-scenario.golden.txt` — NEW v4 golden (embedded, LF, 300 samples).
...
| 2026-06-25 | 1.0 | `gds-dev-story` implementation — all 8 tasks, all 6 ACs. Enum +4 (`AttackTarget`/`Patrol`/`Follow`/`PatrolAppend`) + `CommandTarget` + patrol-route ring SoA; CombatSystem Hold/Stop split + 4 new Tick branches; MovementSystem Hold-anchor; the two apply switches unified behind a single Godot-free `OrderApplier.Apply` (replay-vs-live parity made structural); SelectionSystem right-click-enemy / P / F; SimChecksum fold + `AlgoVersion 3→4`, 3 version-pin guards updated (known-state re-pinned `0x964FF9F8`), all 5 goldens re-recorded to v4 + new cross-platform-safe `command-vocabulary` golden (300 samples). +24 Tier-1 tests → **263 pass / 1 skip / 0 fail**; full Godot build 0 errors. Status → review. | Link Freeman (Dev) |
| 2026-06-25 | 1.1 | `gds-code-review` (3-layer adversarial — Blind Hunter / Edge Case Hunter / Acceptance Auditor, fresh-context Opus — + independent Tier-1 re-run) → **PASS**. 1 decision (AttackTarget force-fire faction guard → Alec chose Option A) + 6 patches applied & verified: same-faction/self guard on AttackTarget/Follow targets; patrol-route reset on leaving Patrol (kills the zombie-route resurrection via `ClearPatrolRoute`); offline AttackTarget/Follow routed through the shared `OrderApplier` (was hand-rolled); S/H clear the patrol/follow/attack-move arm flags; +1 AC2 in-range force-fire-damage test; defensive checksum waypoint clamp; cross-platform-gate doc wording. 2 deferred (Patrol/Follow nav-path per Decision #4; zero-damage Patrol/Follow = Epic-2), 3 dismissed (verified non-issues: wire-id OOB, Hold-hash-drop, golden-rerecord false alarm). Tier-1 **264 pass / 1 skip / 0 fail** (+1), all goldens byte-identical (no re-record), Godot build 0 errors. Status → done. | Reviewer (gds-code-review) |

## Review Findings (gds-code-review 2026-06-25)

**Verdict: PASS.** Adversarial 3-layer review (Blind Hunter / Edge Case Hunter / Acceptance Auditor — all Opus, fresh context) + independent Tier-1 Release re-run (**263 pass / 1 skip / 0 fail**, re-verified, not trusted from the Change Log). All 6 ACs met; every out-of-scope fence honored; determinism contract solid — both apply paths are structurally unified via `OrderApplier` (replay-vs-live divergence is now impossible), the checksum fold is count-driven + `IsAlive`-guarded, no `float` in sim, `IsAlive` is bounds-safe, the patrol reverse-at-ends math is correct for N=2/N=3, and recycled slots reset in `Create()`. **No Critical/High; no desync/determinism defects.** The findings below are gameplay-correctness edges + polish — none block the AC set.

### Decision needed
- [x] [Review][Decision] **RESOLVED 2026-06-25 → Option A** (Alec): add a same-faction+self guard → reclassified as the first patch below. AttackTarget (and Follow) have no faction/self guard on the *target* — `TickAttackTargetCombat` (`godot/src/Combat/CombatSystem.cs:218-252`) force-fires `CommandTarget` with only an `IsAlive` guard (no `FactionOf` check), and `DamageResolver.Apply` has none either. Reachable via (a) free-list recycle ABA — a killed enemy's slot reused by a friendly/own unit before the attacker's next tick — or (b) a crafted/replayed order, yielding deterministic friendly-fire/self-damage (NOT a desync — all peers agree). The 1.12 golden RELIES on force-firing a **Neutral**, so any fix must block only **same-faction (+ self)**, never all non-enemies, or the golden breaks. Options: **(A)** add a same-faction+self guard → clear slot & fall to Idle [recommended, golden-safe]; **(B)** leave as-is (force-attack may hit anything, as some RTS allow); **(C)** other. `TickFollowCombat` (`:332`) has the analogous gap (Low, benign — tracking-only).

### Patches (unambiguous fixes) — ✅ all 7 applied & verified 2026-06-25
_Tier-1 Release **264 pass / 1 skip / 0 fail** (+1 new AC2 test) · all 6 existing/v4 goldens byte-identical (NO re-record needed — the patches move no checksum) · full Godot Debug build **0 errors**._
- [x] [Review][Patch] (from Decision, Option A) Same-faction+self guard on AttackTarget/Follow targets [godot/src/Combat/CombatSystem.cs:218-252, 332-356] — in `TickAttackTargetCombat`, extend the validity guard so a forced target that is the attacker's own faction OR itself is treated as invalid: clear `CommandTarget[i]=-1`, set `CommandState[i]=Idle`, and fall back to acquire-nearest (same path as the dead-target case). Block only SAME-faction (`FactionOf[forced]==FactionOf[i]`) + self — NOT Neutral — so the golden's Neutral force-fire stays green. Apply the analogous guard to `TickFollowCombat`. Re-verify all goldens after (expected: unchanged, since no golden unit force-fires a same-faction target).
- [x] [Review][Patch] Patrol route state not reset on leaving Patrol → "zombie route" resurrection [godot/src/Multiplayer/NetworkCommand.cs:200-201] — `PatrolAppend`'s "already patrolling?" gate is `PatrolCount >= 2`, but no command clears `PatrolCount` when a unit leaves Patrol. Sequence `P`-click (patrol) → `S` (stop) → `P`-shift-click (append) appends onto the abandoned route with a stale `PatrolIndex`, walking the unit back to its old lane. Fix: reset `PatrolCount=0; PatrolIndex=0; PatrolDir=1` in the Move/AttackMove/Stop/Hold/AttackTarget/Follow cases of `OrderApplier` (also zeroes the benign stale route data that v4 folds into the checksum). Re-verify goldens after.
- [x] [Review][Patch] Offline AttackTarget/Follow hand-roll the apply instead of delegating to `OrderApplier` [godot/src/UI/SelectionSystem.cs:530-533, 571-573] — `IssuePatrolCommand:556` correctly calls `OrderApplier.Apply`; AttackTarget/Follow duplicate the field writes. Equivalent today, but `CommandApplyParityTests` only covers `OrderApplier`, so future drift silently diverges offline single-player from online. Fix: route both offline paths through `OrderApplier.Apply` exactly like Patrol does (also resolves the offline-only self-follow guard inconsistency — `SelectionSystem.cs:569` vs `OrderApplier.Follow`).
- [x] [Review][Patch] `S` / `H` key handlers don't clear the patrol/follow/attack-move arm flags [godot/src/UI/SelectionSystem.cs:285-292] — Q/P/F/Esc all reset the three `_awaiting*Click` flags; S and H don't. `P`-then-`S` leaves patrol armed, so the next left-click is silently consumed as a patrol waypoint instead of a selection. Fix: add the three flag-clears to the S and H branches.
- [x] [Review][Patch] AC2 force-fire test never asserts the forced target takes damage in-range [godot/ProjectChimera.Sim.Tests/Combat/CommandVocabularyTests.cs:80-97] — `AttackTarget_ForceFires_IgnoringNearerEnemy` places the forced target out of range (dist 10, range 2), proving "ignore nearer" but not "damage the forced target in-range" (Task 7's wording). Behavior is correct (covered transitively by the golden). Fix: add a forced-in-range + nearer-in-range case asserting the forced HP drops and the nearer is untouched.
- [x] [Review][Patch] Checksum waypoint loop trusts `PatrolCount` with no clamp [godot/src/Core/SimChecksum.cs] — loops `k < PatrolCount[i]` over a width-`MAX_PATROL_WAYPOINTS` (8) ring; for the last entity, `PatrolCount > 8` would read past the flat array end inside per-tick desync detection. Can't trigger today (`OrderApplier` caps at 8) but it's cheap defensive hardening in a critical path. Fix: clamp `k < min(PatrolCount[i], MAX_PATROL_WAYPOINTS)`.
- [x] [Review][Patch] Doc inconsistency on cross-platform gating [godot/src/Core/SimChecksum.cs] — a comment says the 1.12 golden "MAY join the Win↔Linux gate" while `CommandVocabularyScenario.cs`/`CommandVocabularyGoldenTests.cs` state definitively it IS compared on both CI legs / is NOT Windows-gated. Align the comment to the assertive (correct) statement.

### Deferred (accepted scope / documented)
- [x] [Review][Defer] Patrol/Follow request no nav-path while Move/AttackMove do [godot/src/Multiplayer/NetworkCommand.cs] — deferred, matches story Decision #4 (direct steering is sufficient and is what the golden exercises; the path delegates are null in the headless harness). Live-play obstacle-avoidance polish for later.
- [x] [Review][Defer] Zero-damage units can't Patrol/Follow (the `AttackDamage==0` early-`continue` precedes the command switch) [godot/src/Combat/CombatSystem.cs:88-93] — deferred, explicitly flagged-not-built in the story (Epic-2 support-unit concern), documented in-code, harmless for shipped content.

### Dismissed (verified non-issues)
- **Wire target id → OOB read** — `IsAlive` is bounds-safe (`id < _nextId` short-circuits before indexing) and the seeded transient `AttackTarget` is re-validated/overwritten next tick; no crash path. (Blind Hunter #4, hedged on this exact unknown.)
- **Hold unit dropped from spatial hash** — the hash is rebuilt at the top of `MovementSystem.Tick:36` before the per-unit loop, so the Hold `continue:52` can't drop it; neighbors still steer around it. (Blind Hunter #9.)
- **Sibling goldens not re-recorded** — false alarm: the goldens were intentionally split out of the diff handed to the diff-only layer. All 5 verified flipped to `checksum_algo_version: 4` and the suite passes against them. (Blind Hunter #3.)
