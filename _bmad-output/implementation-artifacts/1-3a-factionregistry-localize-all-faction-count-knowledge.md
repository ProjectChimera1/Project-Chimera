...
_Adversarial code review — Blind Hunter + Edge Case Hunter + Acceptance Auditor, 2026-06-23. Scope: commit `f908a15` (the 1.3a changeset; matches the File List exactly). All three layers ran at Opus capability; none failed. The Acceptance Auditor re-ran the build + full suite: **35/35 green, the 1.2 golden is byte-unchanged, and every scope-fence item was verified untouched.** AC1/AC2/AC3 are functionally satisfied._

**decision_needed → resolved as defer (1):**

- [x] [Review][Defer] `FactionRegistry` ctor accepts `[1, PLAYER_COUNT=8]`, but the sim can only honor 4 today → `new FactionRegistry(5..8)` throws `IndexOutOfRangeException` in `SimChecksum` — The ctor range-validates `activePlayerCount ∈ [1,8]`, but the `Faction` enum only defines `Player1..Player4` and `ResourceStore.Ore` is `Fixed[5]` (`ResourceStore.cs:9`). A registry of 5–8 makes `SimChecksum.Compute`'s `resources.Ore[(int)f]` loop (`SimChecksum.cs:~53`) index out of bounds, and slots 5–8 cast to undefined `(Faction)` members. **Not triggerable today** — every live caller passes ≤4 (MainScene=2, GoldenScenario=2, MultiFactionScenario=4) — and the story deliberately defers the store-array resize to Story 9.2 while instructing validation against `PLAYER_COUNT`. But `FactionRegistryTests.Ctor_AcceptsTheInclusiveBounds(8)` now asserts this unusable state is "valid," cementing the gap. [blind+edge]
  - **Resolved 2026-06-23 → deferred to Story 9.2:** Forward constant by design — Story 9.2 resizes the store arrays (5→9) to make 5–8 players usable and does the exhaustive `(int)Faction` audit. No live caller passes >4, so the gap is documented and tracked, not live.
