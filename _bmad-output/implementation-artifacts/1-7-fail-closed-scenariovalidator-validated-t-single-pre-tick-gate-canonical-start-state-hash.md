...
2. **(Invalid models are rejected with a located error)** **Given** an invalid model — an **out-of-range / non-finite** value (NaN/±Inf, outside the 16.16 `Fixed` range, a position outside `MapBounds`, a negative ore/supply/rate) or a **dangling reference** (a building/unit owned by a `slot` no `PlayerSlot` declares, a duplicate slot, an unknown `Building.Type` name) — **When** `Validate` runs **Then** it returns `ValidationResult` with `Ok == false` and a **located** error string (naming the offending field path, e.g. `scenario.units[3].slot`), and (in fail-closed mode) the model never reaches `ApplyScenario`. _(The epic's third example — "a random effect declared while `SimRng` is absent", AR-13 — is **structurally satisfied today** and cannot be exercised: `SimRng` shipped in 1.5 and is unconditionally present, and no effect schema exists until Epic 2. 1.7 owns the rule's home and reserves it; the mature rule is enforced by Epic 2's effect-validator — see D4.)_
...
- **Forbidden-until-`SimRng` (AR-13) — structurally satisfied; reserved for Epic 2, NOT implemented here.** This rule's literal purpose was to block random effects *until* `SimRng` exists. `SimRng` shipped in **Story 1.5** and is **unconditionally present** on every `EntityWorld` (`world.Rng`, non-null, no flag — verified `EntityWorld.cs:79,194`), and **no effect/ability/random-effect schema exists yet** (Epic 2). So the rule has **no live model to reject** and its failing condition (`SimRng` absent) **can never occur**. Do **NOT** add a `SimRng`-presence check or a truth-table function whose failing branch is physically unreachable — that is speculative scaffolding. Instead, **reserve** the rule with one explicit comment in `Validate` (naming AR-13 + the forward owner). The *mature* form — *a random effect is valid only if it draws from `world.Rng`, never `System.Random`/Godot RNG* — is a static check over the **effect graph**, implemented by **Epic 2's effect-validator (Story 2.3 — "AbilityDefinition data model and `Validated<T>` gate with static validator rules")**, the first point an effect schema exists. 1.7 *owns the rule's home* (the validator) and discharges AR-13 by reservation; AC2's testable weight rests entirely on the **out-of-range** and **dangling-reference** checks above.
...
namespace ProjectChimera.Core.Definitions
{
    public sealed class ScenarioValidator
    {
        ...
        public ValidationResult Validate(ScenarioData m)
        {
            ...
            // AR-13 (forbidden-until-SimRng): structurally satisfied — SimRng shipped in Story 1.5 and is
            // unconditionally present (EntityWorld.Rng), and no effect schema exists yet (Epic 2). Nothing to
            // check here; the mature "random effects must draw from world.Rng" rule is enforced by Epic 2's
            // effect-validator (Story 2.3). Do NOT add a presence check that can never fire.
...
- **Trigger-coverage gap (known, deferred — do not try to close it here).** Excluding `Triggers` from the canonical hash means the lobby handshake **no longer detects a trigger-data-only mismatch** (two models with identical start-state-sim but different triggers) that today's file-byte hash *does* catch. This is the architecture's deliberate staging — Epic 7 / D3.4 folds trigger/DSL data into the canonical hash once the graph IR exists; folding raw `TriggerDefinition` here would reintroduce float-text fragility for trigger thresholds and be thrown away. The gap is narrow and bounded: (a) peers normally load the **same** file → same triggers; (b) it only bites on the identical-sim/different-triggers corner; (c) any **gameplay-affecting** trigger divergence still surfaces via the runtime `SimChecksum` once a trigger mutates hashed sim state (spawns, ore, health) or stalls on a divergent `OnVictory`; (d) the byte-hash (`ScenarioSerializer.ComputeFileHash`, algo-1) stays computed/available. **Caveat (verified):** `ScenarioDirector`'s internal trigger state — `_timers`/`_variables`/`_triggerFired`/`_triggerCooldown` (`ScenarioDirector.cs:29-36`) — is **NOT** in `SimChecksum` (`SimChecksum.cs:9-18`), so a *latent* trigger divergence is caught only when it produces a hashed effect, not at the instant it diverges. Acceptable for 1.7; closed in Epic 7. [Source: SimChecksum.cs:9-18; ScenarioDirector.cs:29-36; game-architecture.md §D3.4]
