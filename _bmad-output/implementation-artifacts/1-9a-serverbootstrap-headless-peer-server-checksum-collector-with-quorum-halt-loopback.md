...
- 2026-06-24 — **Alec confirmed LIVE:** ran `loopback-desync-smoke` (visible server + 2 auto-joining client windows), pressed F9, and **both clients showed the terminal "MATCH HALTED" overlay**. Task 10 / AC4 confirmed by the user in a real 3-window run (on top of the automated headless self-test PASS). Story dev complete — parked at `review` for code-review.
...
- 2026-06-24 — **[Code review]** Adversarial review (Blind Hunter + Edge Case Hunter + Acceptance Auditor, Opus 4.8) of `3599834..f8681f1`. **All 5 ACs satisfied, scope fence clean**; Tier-1 183 green, three existing goldens byte-identical, three new types Godot-free (all re-verified live). **2 patches APPLIED:** (1) `HIGH` — spectator checksums were polluting the server quorum (D6 was only half-implemented); fixed by dropping `slot >= MAX_PLAYERS` in `DedicatedServer`'s `Checksum` case + an `IsSpectator` guard in `LockstepManager.SendChecksum`. (2) `LOW` — terminal HALT overlay mislabeled a real `0x00000000` checksum as a global halt; fixed by threading a `hasCanonical` flag through `OnHalt`/`RaiseHalt`/`ShowHalt`. **2 findings deferred to Epic 9** (quorum peer-set fixed at construction → 9-5 disconnect / 9-2-9-3a N-scale; ring-window late-checksum drop) — logged in `deferred-work.md`. Post-patch: `godot.csproj` 0 errors, Tier-1 183 green, goldens byte-identical. Status review → **done**.
...
_Adversarial review — Blind Hunter + Edge Case Hunter + Acceptance Auditor (all Opus 4.8), triaged against diff `3599834..f8681f1`. **Live verification this run:** Tier-1 **183 green**, the three existing goldens **byte-identical** to baseline, the three new types **Godot-free** (zero `using Godot`/`GD.`), determinism test confirmed at **300 ticks**. **Acceptance Auditor: all 5 ACs SATISFIED, scope fence CLEAN, all traps avoided.** The findings below are robustness/edge issues outside the AC surface; none block the AC verification of the 1v1-loopback story._

### Patch (fixable, unambiguous)

- [x] [Review][Patch] **Spectator checksums pollute the strict-majority quorum — half-implemented D6 (`HIGH`)** [godot/src/Multiplayer/DedicatedServer.cs:173 + godot/src/Multiplayer/LockstepManager.cs:352] — ✅ **FIXED 2026-06-24**: server `Checksum` case now drops `slot >= ServerTransport.MAX_PLAYERS` before feeding the collector, and `LockstepManager.SendChecksum` early-returns on `IsSpectator` (defense-in-depth). Rebuilt `godot.csproj` 0 errors; Tier-1 183 green; goldens byte-identical.
  - A spectator (slot ≥ `MAX_PLAYERS`, `IsOnline=true`) runs the sim — its `Flush` spectator path applies both factions' orders and returns `true` (`LockstepManager.cs:262-285`) — and the checksum sink fires on `IsOnline` (`MainScene.cs:306-309`). `SendChecksum` guards **only `!IsOnline`, not `IsSpectator`** (`LockstepManager.cs:352-358`), so the spectator sends `Checksum` packets. The server's `Checksum` case has **no `slot < MAX_PLAYERS` filter** (`DedicatedServer.cs:173`) and `ServerChecksumCollector.Record` rejects only `slot >= MaxSlots(4)` (`:111`). With `expectedPeerCount=2`, a bucket COMPLETES at `Count >= _expected` (`:133`) from the **wrong reporter set** — e.g. player-0 + spectator-2 reporting before player-1 — so the verdict is tallied over the wrong slots and player-1's real checksum is then dropped as already-resolved (`:112`). Result: a genuine player desync can be **missed entirely**, or a lagging/divergent spectator triggers a **false broadcast HALT**. **Directly contradicts D6** ("its checksums, if any, are *ignored by the collector*") — `expectedPeerCount` excludes spectators but the collector still *accepts their reports*. Not reachable on the 1v1-loopback AC path (no spectator), but spectators are a wired feature (`HandleConnect:104`, `BroadcastCommandsToSpectators`) so any 3rd client connecting corrupts the authoritative guard. **Fix:** gate the server feed on player slots — `if (slot < ServerTransport.MAX_PLAYERS)` before `_serverHost.OnChecksum(...)` — and (defense-in-depth) early-return in `SendChecksum` when `IsSpectator`. (Surfaced by Edge Case Hunter; missed by the Acceptance Auditor; verified live this run.)

- [x] [Review][Patch] **Terminal HALT overlay mislabels an attributed desync with canonical hash `0x00000000` as a global no-majority halt (`LOW`/cosmetic)** [godot/src/Core/MainScene.cs ShowHalt + godot/src/Multiplayer/LockstepManager.cs:367] — ✅ **FIXED 2026-06-24**: threaded a `hasCanonical` bool through `OnHalt` (`Action<uint,uint,bool>`) → `RaiseHalt` → `ShowHalt`; the overlay now branches on `hasCanonical` (true for DesyncAlert, false for global Halt) instead of `canonicalHash != 0u`. Rebuilt 0 errors; Tier-1 183 green.
  - `RaiseHalt(tick, 0u)` is the global-`Halt` call; `ShowHalt` keys on `canonicalHash != 0u` to choose the `#{hash:X8}` vs `@tick` status string. `0` is overloaded to mean both "no canonical (global Halt)" and "the canonical hash is the value 0" — a valid 32-bit checksum. A real `DesyncAlert` carrying canonical `0` renders as the generic "@tick" form. Match still halts terminally and correctly; purely a status-line mislabel. **Fix:** distinguish the two `RaiseHalt` call sites with an explicit `bool hasCanonical` (or a sentinel that isn't a valid hash) instead of overloading `0`. (Surfaced by Blind + Edge Case Hunters.)

### Defer (forward-scoped — Epic 9 / lag policy)

- [x] [Review][Defer] **Quorum peer-set is fixed at construction — mid-match disconnect and N≥3 minority self-halt leave the desync guard silently dead** [godot/src/Multiplayer/DedicatedServer.cs:124 + godot/src/Multiplayer/Server/ServerHost.cs OnChecksum] — deferred, forward-scoped. `_serverHost`/`_expected` never re-base when the live peer set shrinks. (a) `HandleDisconnect` (`:124-143`) resets `_state` but never updates `_serverHost`, and `_state` leaving `InGame` makes the `Checksum` case (`:173`) bypass the collector → the server's desync guard is silently dead for the rest of the match (no HALT, no log beyond the disconnect). (b) At N≥3 a majority-with-minority alerts the minority (`ServerHost.OnChecksum`) but does **not** set `Halted`; the alerted minority stops sending checksums (`Flush` halted) so the fixed-N collector can never complete another bucket and the surviving majority runs on diverged-unaware. Both are explicitly **out of the 1.9a scope fence**: disconnect freeze/continue policy is **Story 9-5**; N≥3 / 8-player is **Story 9-2/9-3a**. 1v1-loopback (the shipping config) constructs N=2, where any disagreement is no-majority → broadcast HALT, so neither path is reachable today. (Surfaced by Blind + Edge Case Hunters; verified live.)

- [x] [Review][Defer] **Collector window: a valid-but-late checksum a full window (8 ticks) behind is silently abandoned — no verdict for that tick** [godot/src/Multiplayer/Server/ServerChecksumCollector.cs:117-120] — deferred, correct for spec'd usage. When tick `T+8` reuses the ring slot of a still-incomplete tick `T` (`T%8 == (T+8)%8`), the partial older bucket is `Reset` (its collected reports discarded) and `_resolvedThrough` is not advanced past `T`; a later report for `T` then hits `b.TickOf > tick` and is dropped as stale (`:119`). A genuine desync on an abandoned tick goes undetected. Only triggers when a peer is ≥8 checksum-intervals behind — which lockstep stalls out first — so it's correct for the documented "≤2 in flight" assumption; tied to the forward lag/disconnect policy (Epic 9). _(Related, negligible: `uint` tick wraparound at ~4.5 yr @30 Hz would also permanently silence the collector — `_resolvedThrough` is a non-wrap-aware `long` compare, `:112`.)_ (Surfaced by Blind + Edge Case + Auditor.)

### Dismissed (false positives / non-defects — recorded, not actionable)

- **`_ready[]` not cleared on disconnect → replacement auto-start** — FALSE: `HandleDisconnect` clears `_ready[slot] = false` (`DedicatedServer.cs:127`). (Blind Hunter, diff-only; refuted live.)
- **`_logSink` null in the headless branch → NRE** — FALSE: `_logSink` is a `readonly` field initialized inline (`MainScene.cs:34 = new GodotLogSink()`), never null. (Blind Hunter, diff-only; refuted live.)
- **`TryReadHalt` accepts an out-of-range `HaltReason` byte** — behavior-safe: any `Halt` is terminal regardless of reason; reader still validates type byte + length. (Edge Case Hunter.)
- **`MakeDesyncAlert` 9-byte wire == `Checksum` length** — disambiguated by the type byte (`0x12` vs `0x11`); `ServerPacketTests.WrongType_ReturnsFalse` proves rejection. (Auditor — satisfied.)
- **AC2 cross-process proof is provenance-based** — same pattern as the 3 existing goldens; the committed golden was recorded by a prior process. Not a defect. (Auditor — satisfied caveat.)
- **F9 hook mutates hashed `_world.Health`** — intended AC4 divergence inducer; `#if DEBUG` + gated on `IsOnline`; never runs in goldens/Tier-1 (verified byte-identical). (Auditor — satisfied.)

### Awareness (scope expansion beyond the spec's drawn boundary — justified, not defects)

- **Root-cause transport fix** (`ENetTransport`/`ServerTransport` → `peer.GetPacket()` instead of `service()` `result[2]`) was not in the spec's enumerated shape-of-work; it surfaced during Task 10 and is **load-bearing for AC4** (without it every data packet was silently dropped). Legitimate pre-existing-bug fix, additive, moves no golden — and it **repairs real MP generally, so Story 1.9b now depends on it.** (Auditor.)
- **Early-Ready fix** in `HandleReady` (record a Ready arriving before both peers connect; start on both-connected-AND-ready) widens D8's "preserve `Ready` exactly," but is a benign correctness fix required for the auto-join loopback smoke; preserves `InGame`/`MakeStartGame`/`SLOT_FACTION` semantics. (Auditor.)
