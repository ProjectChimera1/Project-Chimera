...
- Tier-1 Release build: **0 errors** (3 pre-existing CS8632 warnings in unrelated files).
- Tier-1 Release test run: **389 passed / 1 skipped / 0 failed** (baseline 346 → **+43 new**).
- Full `godot.csproj` production build (Godot SDK): **0 errors** (3 pre-existing warnings).
- Determinism analyzer gate (`ProjectChimera.Sim.Analysis`, **RELEASE `-p:ChimeraRelease=true --no-incremental`**): **exit 0**, 314 advisory warnings — **all pre-existing files** (FlowField/GatheringSystem/CombatSystem/BuildingSystem/AiOpponentSystem/FogOfWar/MovementSystem = the documented D2 AI-float debt etc.); **zero CHM/RS0030 findings in any new 2.3 file**.
- **A3 (prove gates have teeth — inject→observe RED→revert), all confirmed RED then reverted:**
  - AC4 total-work: disabled `MaxTotalEffectNodes` → `TotalNodes64_Passes_65Rejected` RED.
  - AC5 re-entrancy: disabled the `InPersistentPhase` install guard → `ApplyModifier_TopLevel_Passes_InsidePersistentPhase_Rejected` RED.
  - Converter unknown-property: forced `ok=true` in `RejectUnknownProperties` → `UnknownProperty_InsideEffectObject_IsLocatedReject` RED.
  - Sole-minter allow-list: removed `AbilityValidator.cs` from the allow-list → `NewValidated_AppearsOnlyInValidatorAllowList` RED (correctly named `AbilityValidator.cs` as the offender).
  - After all reverts: full suite **389/1/0**; no `[A3-INJECT]` marker remains; goldens + `SimChecksum.cs` untouched.

### Completion Notes List

- **Decisions resolved by Alec (in-flight, layman's terms):** #1 = **A** (deserialize JSON directly into the runtime `EffectNode` types — one canonical representation, also the best fit for the AI-authoring/transparency vision Alec described, captured to memory `[[chimera-ability-authoring-ai-transparency-vision]]`); #2 = **A** (new `AbilityValidator.cs` minting `Validated<AbilityDefinition>`, sole-minter allow-list extended to `{ScenarioValidator.cs, AbilityValidator.cs}`); #3 = **keep `MaxSearchAreaDepth=2` / `MaxTotalEffectNodes=64`**.
- **Converter design (AC2/AC6):** order-independent — each node subtree is read into a transient `JsonDocument` (load-time only), so the `kind` discriminator need not be first AND stray properties are detectable. Field VALUES route through the shared `ContentJson.Options` converters (`Fixed`→`FixedJsonConverter`, the ONE quantizer; enums by NAME via `JsonStringEnumConverter(allowIntegerValues:false)` — numeric enum values are rejected). Unknown properties are rejected at BOTH layers: the POCO via `UnmappedMemberHandling.Disallow`, and INSIDE effect/modifier objects via the converter's own `RejectUnknownProperties` (Disallow does not reach inside a custom converter — the AR-22 fail-closed hole). `Modifier` is read by a private helper in the same converter (one converter owns the whole effect+modifier surface).
- **Parse-depth guard:** the converter's composition-depth check mirrors `EffectBounds`' threshold exactly (root=0; reject a composition at depth ≥ `MaxEffectDepth`), so no valid graph (max depth = `MaxEffectDepth` compositions + a leaf) is ever false-rejected, and a 9-deep JSON throws a LOCATED parse error rather than stack-overflowing (belt-and-suspenders alongside `JsonSerializerOptions.MaxDepth`=64, which bounds the DOM build itself).
- **Validator (AC4/AC5):** one iterative explicit-stack walk carries per-path state (`searchAreaDepth`, `inPersistentPhase`, `inPersistentPeriod`) + a total-node counter — composing on top of the reused `EffectBounds.Validate` (depth/width). Top-level `ApplyModifier`/`Persistent` are ACCEPTED (both execute since 2.2b); only the nested-in-Persistent-phase re-entrancy and `SearchArea`-in-period cases are rejected. Located errors are `ability '<id>'.<path>: <reason>`.
- **`ParsedTargeting` is nullable** (returns `null` for an unknown string) so the validator rejects unknown targeting LOCATED rather than silently defaulting (a deliberate, documented divergence from `UnitDefinition.ParsedCategory`, which defaults unknown→Melee — an ability's targeting mode changes how it is cast).
- **Loader:** never returns null / never lets a `JsonException` escape; on a parse error it best-effort peeks the top-level `id` (via a throw-safe `JsonDocument.Parse`) for the located prefix, else falls back to the `sourceLabel`.
- **Determinism posture (the load-bearing invariant): NO fold** — 2.3 is load-time only and adds no new mutable per-entity SoA (cast is 2.4). `SimChecksum.AlgoVersion` stays **6**, `SimChecksum.cs` untouched, **all 8 goldens byte-identical**, `SystemOrderTest` untouched. AC1's "golden checksum" is met by a two-run `SimChecksum.Compute` equality unit test (+ a non-vacuous negative control) — no persisted `.golden.txt`. [[chimera-checksum-fold-timing-rule]]
- **AR-13 discharged by reservation** (AC3): no random leaf is built; a `random_*`/`lua`/`run_script` kind is rejected as unknown by the closed registry; the no-script guarantee is structural (no node carries a Delegate/object field); `EntityWorld.Rng` is unconditionally present.
- **`SimSources.props` needed NO edit** — `src/Core/**` + `src/Effects/**` are already globbed into both the Tier-1 harness and the analyzer gate (Task 1.4 verified), so the new files are auto-covered by both.
- **Deferrals logged** to `deferred-work.md` §"story 2.3" (Write/round-trip→2.5; `ContentJson` consolidation→D3; runtime cast→2.4; cap fold→Epic-9; the `Modifier.period_effect`-SearchArea silent-no-op nuance; the 2.2b content-warning items). **2.3 DISCHARGED** the 2.2b-review re-entrancy item + 2.2b carve-off 1 + the 2.1-review total-work/admissibility items (recorded there).

### File List

**New (sim — `godot/src/Core/Definitions/`):** `AbilityDefinition.cs`, `AbilityTargeting.cs`, `ContentJson.cs`, `EffectNodeJsonConverter.cs`, `AbilityValidationResult.cs`, `AbilityValidator.cs`, `AbilityLoader.cs`
**Modified (sim):** `godot/src/Effects/EffectCaps.cs` (+`MaxSearchAreaDepth`, +`MaxTotalEffectNodes`)
**New (data):** `godot/resources/data/abilities/{fireball.json, minor_heal.json, battle_fury.json}`
**New (tests — `godot/ProjectChimera.Sim.Tests/Definitions/`):** `AbilityDeserializeTests.cs`, `EffectNodeConverterTests.cs`, `NegativeAbilityValidationTests.cs`, `AbilityDeterminismTests.cs`, `AbilityMintingTests.cs`, `AbilityScriptReservationTests.cs`
**Modified (test):** `godot/ProjectChimera.Sim.Tests/Validation/ValidatedMintingTests.cs` (sole-minter allow-list `+= AbilityValidator.cs`; method renamed `NewValidated_AppearsOnlyInValidatorAllowList`; `+using System.Collections.Generic`)
**Modified (tracking):** story file, `_bmad-output/implementation-artifacts/sprint-status.yaml`, `_bmad-output/implementation-artifacts/deferred-work.md`

### Change Log

| Date | Change |
|---|---|
| 2026-06-26 | Story 2.3 **CODE-REVIEWED** (`gds-code-review`, 3-layer adversarial — Blind / Edge / Acceptance Auditor on Opus + orchestrator code verification) → **PASS, status `done`**. All 6 ACs re-verified MET; determinism posture intact (AlgoVersion 6, all 8 goldens byte-identical, `SimChecksum.cs`/`SystemOrderTest` untouched). **1 HIGH decision + 7 patches APPLIED, 1 dismissed.** Headline (HIGH, Alec chose fix-now): the validator & `EffectBounds` treated `ApplyModifierEffect` as a leaf and never walked `Modifier.period_effect` — which `ModifierStore.Advance` runs per-tick on its shared-stack dedicated executor — so an install-leaf there (the W1 re-entrancy hazard `ModifierStore`'s docstring states the 2.3 validator prevents) PASSED validation; `deferred-work` item 5 had disclosed only the harmless `SearchArea`-no-op sub-case, not this corruption sub-case. Fix: `WalkGraph` now descends into `modifier.period_effect` (inPersistentPhase + inPersistentPeriod = true). Other patches: `SearchArea` in Persistent initial/expire silently no-op'd (AC5(b) now spans all 3 phases) · `Fixed.ToFloat()`→`.Raw` in 2 error strings (cleared 2× advisory CHM0005 — the dev's "0 CHM in new files" claim is now TRUE) · loader null-json/file-IO leak now fail-closed · duplicate-JSON-key reject · reserved `TargetFilter` bits (Air/Ground/Structure) fail-closed (were fail-OPEN) · empty-`sequence` reject · located-error STJ-path normalization. +7 teeth tests. Tier-1 **396 pass / 1 skip / 0 fail** (+7); goldens byte-identical (no fold). Dismissed: `period_ticks:0` div-by-zero (false positive — `ModifierStore` guards `PeriodTicks > 0`). |
| 2026-06-26 | Story 2.3 **DEV-DONE** (`gds-dev-story`, claude-opus-4-8) → `review`. Built the AR-22 ability authoring surface: `AbilityDefinition` model + `AbilityTargeting` enum + the closed-registry `EffectNodeJsonConverter` (JSON → the 8 runtime 2.1 `EffectNode` types directly, Decision #1=A; no `[JsonPolymorphic]`) + canonical `ContentJson.Options` (`Disallow` + 3 converters, enums name-only) + `AbilityValidator`→`Validated<AbilityDefinition>` (Decision #2=A, sole-minter allow-list extended) composing reused `EffectBounds` with the 3 new gate families (AC4 `MaxSearchAreaDepth=2`/`MaxTotalEffectNodes=64`, AC5 re-entrancy/period-shape, AC6 fail-closed loader) + fail-closed `AbilityLoader` + 3 sample ability JSONs. **All 6 ACs met across 6 tasks.** Determinism posture: NO fold (AlgoVersion stays 6, all 8 goldens byte-identical, `SimChecksum.cs`/`SystemOrderTest` untouched). Tier-1 **389 pass / 1 skip / 0 fail** (+43); full Godot build 0 errors; analyzer release gate exit 0 (0 CHM/RS0030 in new files); A3 teeth proven for all 4 new gates (inject→RED→revert). AR-13 discharged by reservation (AC3). Deferrals + discharged carve-offs logged to `deferred-work.md` §"story 2.3". Status → review. |
...
_3-layer adversarial review (Blind Hunter / Edge Case Hunter / Acceptance Auditor — all Opus, context-free) + orchestrator code verification against the live `ModifierStore`/`EffectExecutor`/`EffectBounds`. **All 6 ACs verified MET as written; determinism posture intact** (AlgoVersion 6, all 8 goldens byte-identical, `SimChecksum.cs`/`SystemOrderTest`/the 8 effect types/`Modifier.cs`/scenario+faction loaders untouched; Decisions #1/#2/#3 honored; sole-minter allow-list correct). The items below are **gate-completeness gaps** the ACs' literal text did not require but the runtime reality does — found by the Blind+Edge layers in the half of the deferral the Auditor (and the dev's `deferred-work` item 5) accepted at face value._

### Decision needed

- [x] [Review][Decision] **[RESOLVED — Alec chose fix-now; closed in 2.3] HIGH — Validator never descends into `ApplyModifierEffect` → `Modifier.PeriodEffect`; the install-leaf re-entrancy sub-case is undisclosed.** `AbilityValidator.WalkGraph` and `EffectBounds.Validate` both treat `ApplyModifierEffect` as a structural leaf and never traverse `Modifier.PeriodEffect` — yet `ModifierStore.Advance` runs that subtree per-tick on its single shared-stack dedicated `EffectExecutor`. So an `ApplyModifier`/`PersistentEffect` placed inside a `modifier.period_effect` passes validation, then on each pulse re-enters the dedicated executor (clobbers the shared pre-allocated `_stack`) and mutates `_count` mid-`Advance` — the **exact W1 re-entrancy hazard `ModifierStore.cs:31-40` states the Story-2.3 validator prevents.** Deterministic corruption (NOT a cross-peer desync), **latent until 2.4 wires casting**, reachable by mainstream content (a DoT that also applies a debuff). `deferred-work` item 5 discloses ONLY the harmless `SearchArea`-in-modifier-period sub-case ("silent no-op, not a crash or desync"); the install-leaf re-entrancy sub-case is undisclosed and item 5's safety argument (parse-guard + frame-cap) addresses *hang*, not *re-entrancy*. (Also corrects story Dev-Notes line 149: `EffectBounds` does NOT descend into `ApplyModifier` — it is the `default` leaf case.) Fix: in `WalkGraph`'s `ApplyModifierEffect` case push `modifier.PeriodEffect` as a frame with `inPersistentPhase=true, inPersistentPeriod=true`. [godot/src/Core/Definitions/AbilityValidator.cs:104-109; godot/src/Effects/EffectBounds.cs:90-92; godot/src/Effects/ModifierStore.cs:245,481-482]

### Patches

- [x] [Review][Patch] LOW-MED — `SearchArea` inside `PersistentEffect.InitialEffect`/`ExpireEffect` passes validation but silently no-ops (store runs all 3 phases `spatial: null`; validator rejects `SearchArea` only on `InPersistentPeriod`, not the broader `InPersistentPhase`). Fix: gate the AC5(b) `SearchArea` rejection on `InPersistentPhase`. [godot/src/Core/Definitions/AbilityValidator.cs:133]
- [x] [Review][Patch] LOW — `AbilityValidator` calls `Fixed.ToFloat()` in two error strings → 2× advisory CHM0005 (ban on `ToFloat` outside `FixedJsonConverter`); falsifies the Dev-Record "0 CHM/RS0030 findings in any new 2.3 file" claim. Fix: print `.Raw` (mirrors the sibling int-cost branches). [godot/src/Core/Definitions/AbilityValidator.cs:51,57]
- [x] [Review][Patch] LOW — `AbilityLoader` leaks non-`JsonException` past its "never throws past its boundary" contract: `Load(null, …)` → `ArgumentNullException`; `LoadFromFile` on a locked/permission-denied/TOCTOU-deleted file → `IOException`/`UnauthorizedAccessException`. Fix: null-guard `json` + try/catch the file read → located `Fail`. [godot/src/Core/Definitions/AbilityLoader.cs:27,54]
- [x] [Review][Patch] LOW — Duplicate JSON property names silently accepted (first-wins): `RejectUnknownProperties` checks names against the allow-list but never counts, and readers use `TryGetProperty` (first match) — a second `amount`/`kind` is dropped. Contradicts AR-22 fail-closed. Fix: fail on any property name seen more than once. [godot/src/Core/Definitions/EffectNodeJsonConverter.cs:258]
- [x] [Review][Patch] LOW — Reserved `TargetFilter` bits (`Air`/`Ground`/`Structure`) pass validation and fail **OPEN**: `TargetMatcher` ignores them → `wanted == None` → matches ALL candidates ("enemy air" silently becomes "all"). Fix: reject `filter & (Air|Ground|Structure) != 0` until 2.9a wires their evaluation. [godot/src/Core/Definitions/EffectNodeJsonConverter.cs (search_area filter)]
- [x] [Review][Patch] LOW — Empty `sequence` (0 children) passes validation as a no-op ability; the story testing-discipline explicitly listed "Sequence 0 children → reject" but it is untested/unhandled. Fix: reject a `SequenceEffect` with `Children.Length == 0`. [godot/src/Core/Definitions/AbilityValidator.cs (WalkGraph)]
- [x] [Review][Patch] LOW (optional) — Located-error contract not honored for `Disallow`/raw-syntax `JsonException`s: the loader concatenates raw System.Text.Json prose (`ability '<id>'.<STJ message> … Path: $.x`) rather than the promised `ability '<id>'.<path>: <reason>`. Teeth still pass (substring asserts). Fix: normalize the message for the unmapped-member/syntax paths. [godot/src/Core/Definitions/AbilityLoader.cs:30]

### Dismissed (1)

- **Modifier numeric fields (`period_ticks:0`, negative `max_stacks`/`period_count`, `duration_ticks==0`) → div-by-zero / infinite-pulse** — FALSE POSITIVE. `ModifierStore.ResetPeriodSchedule`/`HasPeriod` guard `PeriodTicks > 0` (period simply never fires; no div-by-zero, no infinite loop — verified by the Edge layer and orchestrator). The residual "nonsensical values pass validation" is already a documented content-warning deferral (`deferred-work` story-2.3 item 6).
