...
- [ ] [Review][Decision] **HIGH — Validator never descends into `ApplyModifierEffect` → `Modifier.PeriodEffect`; the install-leaf re-entrancy sub-case is undisclosed.** `AbilityValidator.WalkGraph` and `EffectBounds.Validate` both treat `ApplyModifierEffect` as a structural leaf and never traverse `Modifier.PeriodEffect` — yet `ModifierStore.Advance` runs that subtree per-tick on its single shared-stack dedicated `EffectExecutor`. So an `ApplyModifier`/`PersistentEffect` placed inside a `modifier.period_effect` passes validation, then on each pulse re-enters the dedicated executor (clobbers the shared pre-allocated `_stack`) and mutates `_count` mid-`Advance` — the **exact W1 re-entrancy hazard `ModifierStore.cs:31-40` states the Story-2.3 validator prevents.** Deterministic corruption (NOT a cross-peer desync), **latent until 2.4 wires casting**, reachable by mainstream content (a DoT that also applies a debuff). `deferred-work` item 5 discloses ONLY the harmless `SearchArea`-in-modifier-period sub-case ("silent no-op, not a crash or desync"); the install-leaf re-entrancy sub-case is undisclosed and item 5's safety argument (parse-guard + frame-cap) addresses *hang*, not *re-entrancy*. (Also corrects story Dev-Notes line 149: `EffectBounds` does NOT descend into `ApplyModifier` — it is the `default` leaf case.) Fix: in `WalkGraph`'s `ApplyModifierEffect` case push `modifier.PeriodEffect` as a frame with `inPersistentPhase=true, inPersistentPeriod=true`. [godot/src/Core/Definitions/AbilityValidator.cs:104-109; godot/src/Effects/EffectBounds.cs:90-92; godot/src/Effects/ModifierStore.cs:245,481-482]

### Patches

- [ ] [Review][Patch] LOW-MED — `SearchArea` inside `PersistentEffect.InitialEffect`/`ExpireEffect` passes validation but silently no-ops (store runs all 3 phases `spatial: null`; validator rejects `SearchArea` only on `InPersistentPeriod`, not the broader `InPersistentPhase`). Fix: gate the AC5(b) `SearchArea` rejection on `InPersistentPhase`. [godot/src/Core/Definitions/AbilityValidator.cs:133]
- [ ] [Review][Patch] LOW — `AbilityValidator` calls `Fixed.ToFloat()` in two error strings → 2× advisory CHM0005 (ban on `ToFloat` outside `FixedJsonConverter`); falsifies the Dev-Record "0 CHM/RS0030 findings in any new 2.3 file" claim. Fix: print `.Raw` (mirrors the sibling int-cost branches). [godot/src/Core/Definitions/AbilityValidator.cs:51,57]
- [ ] [Review][Patch] LOW — `AbilityLoader` leaks non-`JsonException` past its "never throws past its boundary" contract: `Load(null, …)` → `ArgumentNullException`; `LoadFromFile` on a locked/permission-denied/TOCTOU-deleted file → `IOException`/`UnauthorizedAccessException`. Fix: null-guard `json` + try/catch the file read → located `Fail`. [godot/src/Core/Definitions/AbilityLoader.cs:27,54]
- [ ] [Review][Patch] LOW — Duplicate JSON property names silently accepted (first-wins): `RejectUnknownProperties` checks names against the allow-list but never counts, and readers use `TryGetProperty` (first match) — a second `amount`/`kind` is dropped. Contradicts AR-22 fail-closed. Fix: fail on any property name seen more than once. [godot/src/Core/Definitions/EffectNodeJsonConverter.cs:258]
- [ ] [Review][Patch] LOW — Reserved `TargetFilter` bits (`Air`/`Ground`/`Structure`) pass validation and fail **OPEN**: `TargetMatcher` ignores them → `wanted == None` → matches ALL candidates ("enemy air" silently becomes "all"). Fix: reject `filter & (Air|Ground|Structure) != 0` until 2.9a wires their evaluation. [godot/src/Core/Definitions/EffectNodeJsonConverter.cs (search_area filter)]
- [ ] [Review][Patch] LOW — Empty `sequence` (0 children) passes validation as a no-op ability; the story testing-discipline explicitly listed "Sequence 0 children → reject" but it is untested/unhandled. Fix: reject a `SequenceEffect` with `Children.Length == 0`. [godot/src/Core/Definitions/AbilityValidator.cs (WalkGraph)]
- [ ] [Review][Patch] LOW (optional) — Located-error contract not honored for `Disallow`/raw-syntax `JsonException`s: the loader concatenates raw System.Text.Json prose (`ability '<id>'.<STJ message> … Path: $.x`) rather than the promised `ability '<id>'.<path>: <reason>`. Teeth still pass (substring asserts). Fix: normalize the message for the unmapped-member/syntax paths. [godot/src/Core/Definitions/AbilityLoader.cs:30]
