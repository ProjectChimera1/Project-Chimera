...
- [x] **Task 3 — Insert `ModifierSystem` at index 3; repoint the live readers to `Effective*` (AC1)**
  ...
  - [x] 3.3 Update `SystemOrderTest` (`Sim/SystemOrderTest.cs`) — **the AC1 enforcer, an intentional update:** add `typeof(ModifierSystem)` at `ExpectedOrder[3]` (10 entries); rename `Systems_AreTheNineCanonicalSystems...`→`...TenCanonicalSystems...`; rewrite `ReservedModifierSlot_CombatSystem_IsImmediatelyPrecededByMovementSystem` → assert `ModifierSystem` is immediately before `CombatSystem` AND immediately after `MovementSystem` (contiguous `Movement, Modifier, Combat`) AND strictly before `ProjectileSystem`. Add `using ProjectChimera.Effects;`.
  - [x] 3.4 Grep for any OTHER system-count/order pin and update (`"nine"`, `"9 system"`, `systems.Count == 9`). The goldens run through the host and pick up the no-op `ModifierSystem` — confirm byte-identical (Task 6.2).

- [x] **Task 4 — Single-mapper SoA rule + guard (retro action item A2) (AC4)**
  - [x] 4.1 **Written rule** — add to `_bmad-output/project-context.md` ("Data layout") AND `godot/CLAUDE.md` ("Simulation Layer"): *"Every new per-unit SoA field that derives from `UnitDefinition` MUST be written via `EntityWorld.ApplyUnitDefinition` (the single def→SoA mapper) — never hand-copied in a spawn path. Stats sourced from the `Create()` ctor args (Health/MaxHealth/MoveSpeed) flow through that single channel; non-def fields are defaulted in `Create()`. This closes the 1.12/1.13 spawn-path/zombie-state defect class."*
  - [x] 4.2 **Tier-1 guard test** (Godot-free) — `ProjectChimera.Sim.Tests/Core/ApplyUnitDefinitionGuardTest.cs` (or extend `Builder/ScenarioApplierTests.cs`):
...
- **Pass 1 (rename gate, the determinism-critical proof).** After renaming the three EntityWorld arrays (`AttackDamage`→`EffectiveAttackDamage`, `MaxHealth`→`EffectiveMaxHealth`, `Speed`→`EffectiveMoveSpeed`) everywhere and BEFORE adding any `Base*` array: Tier-1 `307 pass / 1 skip / 0 fail` (exact pre-story baseline), full Godot build `0 errors`, and `git status --short -- '*.golden.txt'` clean. This isolates the rename as behavior-preserving (the #1 review concern) independently of the additive pass.
- **Pass 2 (additive).** After adding `Base*`/`Energy`/`MaxEnergy`, the no-op `ModifierSystem` at host index 3, the new tests, and the doc/guard work: Tier-1 `314 pass / 1 skip / 0 fail` (+7), Godot build `0 errors`, goldens byte-identical, `SimChecksum.cs` untouched, `AlgoVersion == 5`, coverage-guard known-state hash `0x5E7BE3D8` unchanged.
- **A2 guard teeth (Task 4.3, inject→observe→revert).** Temporarily commented out `world.ApplyUnitDefinition(id, def)` in `ScenarioApplier.SpawnUnit` → `ApplyUnitDefinitionGuardTest.SpawnUnit_DefDerivedFields_MatchCreatePlusApplyUnitDefinition` went **RED** at the `BaseAttackDamage` assertion (expected raw `1114112` = `Fixed.FromFloat(17)`, actual `0` = the Create default a forgotten mapper leaves). Reverted → GREEN. Confirms the parity guard catches a spawn path that forgets the single mapper.

### Completion Notes List

- **Two-pass, behavior-preserving build (story §"the safe two-pass sequence").** Pass 1 = pure rename of the three modifier-affected EntityWorld arrays to `Effective*` (compiler found every site across sim, presentation, golden scenarios, and tests); proven golden-neutral by the Pass-1 gate above. Pass 2 = additive `BaseAttackDamage`/`BaseMaxHealth`/`BaseMoveSpeed` + `Energy`/`MaxEnergy` (8 new arrays), `Create` dual-writes `Base*` from the ctor args / Zero, and `ApplyUnitDefinition` splits the one mapper-sourced stat (`BaseAttackDamage = FromFloat(def.AttackDamage)` → `EffectiveAttackDamage = BaseAttackDamage`).
- **AC1 — `ModifierSystem` at index 3, live readers on `Effective*`.** New `src/Effects/ModifierSystem.cs` (`ISimSystem`) inserted at `SimulationHost` index 3 (Movement@2 → **Modifier@3** → Combat@4 → Projectile@5; shifted the tail by one). `CombatSystem` (combatant guard + projectile-spawn arg + melee arg), `MovementSystem` (move-speed reads), and `HealEffect`/`DirectHpDeltaEffect` (Health-clamp ceiling) all read `Effective*` (via the rename). `ProjectileSystem` unchanged (snapshots `Effective` at spawn; runs after Modifier). `SystemOrderTest` updated to the 10-system order + a contiguity assertion (Movement, Modifier, Combat adjacent; Modifier strictly before Projectile). Proven by `CombatReadsEffectiveTests` (a +40 buff ⇒ combat deals 50, not Base 10; Hero/Hero ×1.0 so the delta is the effective damage exactly).
- **AC2 — dirty-flag recompute correct, idempotent, order-independent.** `ModifierSystemTests`: recompute = `Base + Σ deltas` with the flag cleared; a clean entity is never recomputed (corrupt-then-Tick survives — the dirty-gate teeth); two deltas commute (`+5,+3 == +3,+5`); dead slots are skipped without throwing. In 2.2a production nothing calls `AccumulateBonus`, so `Tick` is a no-op and `Effective* == Base*` everywhere.
- **AC3 — NO checksum fold, goldens byte-identical (the load-bearing invariant).** `SimChecksum.cs` untouched, `AlgoVersion` stays **5**, all **7** `*.golden.txt` byte-identical (`git status` clean), `SimChecksumCoverageGuardTest` green incl. the pinned `0x5E7BE3D8`. The 8 new arrays are dormant (spawn-constant / `Effective==Base` / `Energy==0`), exactly like `AttackDamage`/`MaxHealth`/`Speed` were never hashed; the single fold + `AlgoVersion 5→6` re-baseline is 2.2b's (per [[chimera-checksum-fold-timing-rule]]). `SystemOrderTest` updated intentionally — a system-array change, not a world-state change.
- **AC4 — single-mapper SoA rule (retro A2) landed.** `ApplyUnitDefinition` is the sole def→SoA writer for `BaseAttackDamage`. The rename turned every non-mapper hand-set into a compile error; each was fixed to set BOTH `Base*` and `Effective*` (the two no-def fallbacks `BuildingSystem.SpawnTrainedUnit` / `EntityPlacer.DoSpawnCombatUnit`, and `EntityPlacer.RestoreUnit`). Rule written to `_bmad-output/project-context.md` (Data layout) and `godot/CLAUDE.md` (Simulation Layer). Guard test `ApplyUnitDefinitionGuardTest` covers (a) mapper completeness and (b) `ScenarioApplier.SpawnUnit` parity vs `Create`+`ApplyUnitDefinition`; teeth proven (see Debug Log).
- **AC4 scope note — `BuildingSystem.SpawnTrainedUnit` not Tier-1-driven.** It is `private`, fires only on production completion, and no existing Tier-1 harness drives it. Per Task 4.2c it is covered by the compiler-forced Base+Effective edit in Task 1.6 (a forgotten new field is a compile error there) + the written single-mapper rule, rather than a bespoke production-completion harness. `EntityPlacer.{DoSpawnCombatUnit/Worker,RestoreUnit}` are `using Godot;` → same compiler-forced coverage.
- **Deferrals recorded** in `deferred-work.md` §"Deferred from: story 2.2a": the single SimChecksum fold (2.2b, AlgoVersion 5→6); MaxHealth-buff Health semantics (2.2b); the move-speed 1-tick lag (Movement@2 reads before Modifier@3 — deterministic, invisible in 2.2a); authored `MaxEnergy` on `UnitDefinition` (2.2b/2.3, through the mapper); `ModifierStore` must clear accumulators/dirty on death/recycle (2.2b SoA-recycle trap); `RestoreUnit`/`UnitSnapshot` widening for the new fields (folds with the 1.13 deferral).
- **Verification:** Tier-1 Release `314 pass / 1 skip / 0 fail`; full `godot.csproj` build `0 errors` (7 pre-existing CS8632 warnings only); 7 goldens byte-identical; no `SimChecksum`/golden/`SimSources.props`/`.csproj`/NuGet changes beyond the documented comment updates.

### File List

**New — simulation:**
- `godot/src/Effects/ModifierSystem.cs`

**New — Tier-1 tests:**
- `godot/ProjectChimera.Sim.Tests/Effects/ModifierSystemTests.cs`
- `godot/ProjectChimera.Sim.Tests/Effects/CombatReadsEffectiveTests.cs`
- `godot/ProjectChimera.Sim.Tests/Core/ApplyUnitDefinitionGuardTest.cs`

**Modified — simulation (rename + additive):**
- `godot/src/Core/EntityWorld.cs` (rename to `Effective*`; add `Base*` + `Energy`/`MaxEnergy`; `Create` dual-write/defaults; `ApplyUnitDefinition` split through `Base`)
- `godot/src/Core/Sim/SimulationHost.cs` (insert `ModifierSystem` at index 3; 9→10 comments + log)
- `godot/src/Combat/CombatSystem.cs` (read `EffectiveAttackDamage`)
- `godot/src/Navigation/MovementSystem.cs` (read `EffectiveMoveSpeed`)
- `godot/src/Economy/BuildingSystem.cs` (no-def fallback sets Base+Effective)
- `godot/src/Effects/HealEffect.cs`, `godot/src/Effects/DirectHpDeltaEffect.cs` (clamp ceiling → `EffectiveMaxHealth`)

**Modified — presentation (rename + A2 dual-write):**
- `godot/src/UI/EntityPlacer.cs` (snapshot reads → `Effective*`; fallback + `RestoreUnit` set Base+Effective)
- `godot/src/UI/CommandCardSystem.cs`, `godot/src/UI/SelectionSystem.cs` (health-bar reads → `EffectiveMaxHealth`)
- `godot/tools/StressTest.cs` (read/write `EffectiveAttackDamage`)

**Modified — tests:**
- `godot/ProjectChimera.Sim.Tests/Sim/SystemOrderTest.cs` (10-system order; ModifierSystem@3 contiguity)
- `godot/ProjectChimera.Sim.Tests/Builder/ScenarioApplierTests.cs`, `.../Combat/CommandVocabularyTests.cs` (rename)
- `.../Golden/{GoldenScenario,MultiFactionScenario,AiActiveScenario,SameTickTieBreakScenario,CommandVocabularyScenario}.cs` (rename)

**Modified — comments only (9-system → 10-system):**
- `godot/SimSources.props`, `godot/src/Core/MainScene.cs`, `godot/src/Core/Bootstrap/ScenePhaseOrder.cs`, `.../Golden/GoldenChecksumReplayTests.cs`, `.../Golden/FormationSeparationScenario.cs`

**Modified — docs / artifacts:**
- `_bmad-output/project-context.md` (A2 single-mapper rule), `godot/CLAUDE.md` (A2 rule), `_bmad-output/implementation-artifacts/deferred-work.md` (2.2a carve-offs)

_Not source edits: several Godot-generated `*.cs.uid` artifacts and `Snapshot.md` were touched by the editor/autosave; no `SimChecksum.cs`, `*.golden.txt`, `*.csproj`, or NuGet changes._
