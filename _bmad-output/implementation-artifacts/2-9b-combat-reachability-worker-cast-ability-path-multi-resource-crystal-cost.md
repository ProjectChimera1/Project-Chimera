...
- **AC1.4 / AC4 human-confirmed live cast (2026-07-02, follow-up):** to make a *successful* cast testable, P1 and P2 were subsequently seeded with 100 starting crystal (new data-driven `start_crystal` field on `alpha_map_01.json` + `ScenarioApplier`/`SettingsData` edits — committed SEPARATELY from this story, not part of the 2.9b diff). With the button now affordable, the developer clicked "Matter Infusion" live and visually confirmed the multi-resource debit — **crystal and energy both dropped** — while the Acolyte kept gathering uninterrupted. Closes the worker-cast + multi-resource-cost path end-to-end in-engine, corroborating the golden/Tier-1 proof. (The greyed `"[need crystal]"` state noted above reflects the pre-seed map; the shipped map now starts P1/P2 with crystal, so re-running this check shows the button lit.)
...
| 2026-07-02 | 1.1 | Code review (`gds-code-review`, Opus 4.8) — **PASS**. 3-layer adversarial (Blind · Edge Case · Acceptance, fresh context) + lead verification against source; scope expanded to the bundled `start_crystal` follow-up. **2.9b proper clean** (all 5 ACs, 6 Decisions, SimChecksum fence intact — AlgoVersion 8, pin `0x983D39AE`, 13 goldens byte-identical). One latent MP-desync gap in the follow-up (`StartCrystal` seeded into sim but unfolded in `CanonicalModelHash`) **FIXED** — `CanonicalModelHash` AlgoVersion 2→3 folds `StartCrystal`, VersionStamp 8/3/1/2, applier pin re-recorded, new `ChangedStartCrystal_HashDiffers` teeth-test; + train-tooltip crystal patch applied. 5 defers + 3 dismissed logged. Tier-1 574/1/0, godot.csproj 0 err, release gate 0 err. Status → done. | Claude (gds-code-review, Opus 4.8) |

---

### Review Findings

_Code review (`gds-code-review`, 2026-07-02) — 3-layer adversarial (Blind Hunter · Edge Case Hunter · Acceptance Auditor, all Opus 4.8, fresh context) + independent lead verification against source at HEAD. **Review scope was expanded by Alec to include the separately-committed `start_crystal` follow-up** (11 files incl. the determinism-fence file `CanonicalModelHash.cs`), since it touches the canonical model and would otherwise slip through unreviewed._

**Verdict — Story 2.9b proper is CLEAN.** All 5 ACs met, all 6 baked Decisions honored, every Regression risk respected; determinism fence intact for 2.9b's own changes (no new SoA field, no fold, `AlgoVersion` stays 8, pin `0x983D39AE` unchanged, all 13 existing goldens byte-identical, `SimChecksum.cs` untouched, 1 new golden). Every finding below is in the bundled `start_crystal` follow-up or is cosmetic/coverage — **none is a 2.9b AC failure.**

- [x] [Review][Decision → FIXED 2026-07-02] **`StartCrystal` is sim-affecting but excluded from `CanonicalModelHash` — latent lockstep-MP desync gap** — `ScenarioApplier.cs:88` seeds `AddCrystal(faction, Fixed.FromFloat(slot.StartCrystal))` and `Crystal` is folded into `SimChecksum` (v8), but `CanonicalModelHash.Compute` (`CanonicalModelHash.cs:53-63`) never reads `StartCrystal` and `AlgoVersion` stays `2`. Two lobby clients differing only in `start_crystal` hash-**equal** in the start-state handshake, then **desync in-sim from tick 1**. `alpha_map_01.json` now ships `start_crystal:100` (both slots) — the first content to exercise this. Flagged independently by ALL 3 review layers; verified firsthand. **LATENT** (lockstep MP inactive; handshake fail-open today) and **documented at the fix site** (`CanonicalModelHash.cs:19-23`) with an exact recipe: fold `StartCrystal` (+ its sort key), bump `AlgoVersion 2→3`, re-record the two pinned baselines (`CanonicalModelHashTests` byte-exact + `ScenarioApplierTests`) in one commit — BEFORE lockstep-MP makes the handshake load-bearing. **NOT a story-2.9b defect** (it is the bundled follow-up). Decision: fix now vs. keep the documented deferral.
- [x] [Review][Patch] Train-button tooltip omits crystal cost — the button face + `[need crystal]` note are already correct (AC2.2 met), but `TooltipText` still reads only "{ore} ore, {N}s train" for a crystal-costed unit [godot/src/UI/CommandCardSystem.cs:~293]
- [x] [Review][Defer] Negative unit `cost_crystal` makes `SpendCrystal` an addition (grants crystal) [godot/src/Economy/BuildingSystem.cs:341 / godot/src/Core/ResourceStore.cs:75] — deferred, pre-existing (symmetric with the unguarded `cost_ore`; the lenient faction/unit loader validates no unit costs)
- [x] [Review][Defer] `start_crystal` ≥ ~32768 overflows the 16.16 `Fixed` (validator runs `CheckNonNeg` only, no upper cap) [godot/src/Core/Definitions/ScenarioValidator.cs:~115] — deferred, pre-existing (symmetric with the identical `start_ore` gap; cf. deferred-work story-1.3b #1)
- [x] [Review][Defer] Shipped `matter_infusion` (`apply_modifier`/`move_speed_delta`) path never exercised — all unit tests + the golden substitute `AbilityTestAbilities.SelfHeal` [godot/ProjectChimera.Sim.Tests/Effects/WorkerCastTests.cs] — deferred, coverage (mechanism proven via the existing ModifierSystem; AC1.4 was proven with a heal, not the speed buff)
- [x] [Review][Defer] Fallback crystal seed `AddCrystal(100)` hardcoded in 3 comment-coupled places with no sync test [godot/src/Core/Sim/ScenarioApplier.cs:159-160] — deferred, maintenance (all three = 100 today; not golden-reachable so CI wouldn't catch a drift)
- [x] [Review][Defer] Cached ability-panel positions computed once from `vpSize` go stale on window resize / can push the stacked card off the top on very short viewports [godot/src/UI/CommandCardSystem.cs] — deferred, pre-existing HUD compute-once pattern

**Dismissed as noise (3):** (1) *"`TrainUnit` discards `Spend*` return values"* — false positive: `SpendOre`/`SpendCrystal` delegate to the identical `CanAffordOre`/`CanAffordCrystal` `>=` comparison, single-threaded, so a passed pre-check guarantees the spend succeeds; atomicity holds (verified firsthand in `ResourceStore.cs:49-79`). (2) *"`EntityPlacer` full-mapper makes placed workers combat-capable"* — intended per AC3; the Acolyte's now-live attack stats are inert (CombatSystem gatherer exemption keys on `GatherState`, not damage). (3) `SimChecksumCoverageGuardTest.cs:34` *"14 goldens stay byte-identical"* wording nit (13 stay + 1 is new) — trivial doc comment, the update was story-mandated (Task 6).

**Scope note (not a defect):** an unrelated `EdgeScrollEnabled` default flip (`true→false`) rode into the working tree via `SettingsData.cs` + `RtsCameraController.cs` — belongs to neither 2.9b nor the `start_crystal` follow-up. Presentation-only, zero sim/determinism impact. Confirm it's intentional; consider isolating it in its own commit.

**Resolution (2026-07-02 — Alec chose "fix both").**

- **[Decision → FIXED] `StartCrystal` fold.** `CanonicalModelHash` now folds `ScenarioPlayerSlot.StartCrystal` (in both the `OrderBy`/`ThenBy` sort key and the mixed byte stream), and its `AlgoVersion` was bumped **2 → 3**. Sibling stamps moved in the same pass: `VersionStampConsistencyTests` now pins **8/3/1/2**; the `ScenarioApplierTests` pinned canonical hash was re-recorded (`12401609732849360762` → `8107768752238097581`); the `CanonicalModelHashTests` byte-exact pin auto-tracked the bump (it reads `AlgoVersion` dynamically); and a **new teeth-test** `ChangedStartCrystal_HashDiffers` proves two models differing only in `start_crystal` now hash **differently** (it would be RED against v2). Effect: two clients with mismatched `start_crystal` are now rejected at the lobby start-state handshake instead of desyncing in-sim from tick 1. **SimChecksum `AlgoVersion` stays 8 and all 13 SimChecksum goldens are byte-identical** — the canonical start-state hash is a separate stamp, so 2.9b's own AC4 fence is unaffected. Files: `CanonicalModelHash.cs`, `VersionStampConsistencyTests.cs`, `CanonicalModelHashTests.cs`, `ScenarioApplierTests.cs`.
- **[Patch → APPLIED] Train tooltip.** `CommandCardSystem`'s train-button `TooltipText` now appends the crystal cost via the existing `costSuffix` (empty for `cost_crystal:0` units — byte-for-byte unchanged, no AC2.3 regression). File: `CommandCardSystem.cs`.
- **Verification:** Tier-1 **574 pass / 1 skip / 0 fail**; full `godot.csproj` build **0 errors**; release analyzer gate (`-p:ChimeraRelease=true --no-incremental`) **0 errors**. The 5 deferred items were logged to `deferred-work.md`. **Status → done.**
